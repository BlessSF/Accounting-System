<?php
// ============================================================
//  demiclab_tb.php — DemicLab-Main Trial Balance
//  ASSETS + REVENUE + EXPENSES × Jan–Dec with TOTAL column
//  Data pulled live from demiclab_disbursement
...
// ── Section definitions ────────────────────────────────────
// ASSETS and EXPENSES now come from demiclab_acc_titles (managed via
// demiclab_acc_title.php) instead of a hardcoded list, grouped by section.
// REVENUE rows are manual P&L revenue types, unrelated to account titles.
$acctRows = $pdo->query("SELECT title, section FROM `demiclab_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_ASSOC);
$ASSETS = []; $EXPENSES = [];
...
if (isset($_GET['export_csv'])) {
    ...
    header('Content-Disposition: attachment; filename="DemicLab-Main_TB_'.$YEAR.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['MERITONI CORP — DemicLab-Main Trial Balance', $YEAR]);
    fputcsv($out, []);
...
<div class="section-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:20px">
  <div>
    <div class="section-title">DemicLab-Main <span>Trial Balance</span></div>
    <div class="section-subtitle">MERITONI CORP · Assets, Revenue &amp; Expenses by month · <?= $YEAR ?></div>
  </div>
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    <a href="demiclab_disbursement.php" class="nav-btn btn-dsb">📒 Disbursement</a>
    <a href="demiclab_sum.php?year=<?= $YEAR ?>" class="nav-btn btn-sum">📋 Sum</a>
    <a href="demiclab_acc_title.php" class="nav-btn" style="background:#6D4C41;color:#fff;box-shadow:0 2px 8px rgba(109,76,65,.3)">🏷 Account Titles</a>
  </div>
</div>
