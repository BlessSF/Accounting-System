<?php
// ============================================================
//  commissary_cogs.php — Commissary Branch COGS (Cost of Goods Sold)
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `commissary_cogs` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        if ($rowId > 0) {
            $pdo->prepare("UPDATE commissary_cogs SET cogs_date=?, cogs_year=?, cogs_month=?, beg=?, purc=?, end_inv=?, cos=?, mktg_cost=?, saved_by=? WHERE id=? AND store_name='Commissary'")
                ->execute([$date,$yr,$mo,$beg,$purc,$end,$cos,$mktg,$user['name'],$rowId]);
            echo json_encode(['ok'=>true, 'id'=>$rowId, 'cos'=>number_format($cos,2)]);
        } else {
            $sql = "INSERT INTO commissary_cogs (cogs_date, cogs_year, cogs_month, store_name, beg, purc, end_inv, cos, mktg_cost, saved_by) VALUES (?,?,?,'Commissary',?,?,?,?,?,?)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM commissary_cogs WHERE id=? AND store_name='Commissary'")->execute([$id]);
...
if (isset($_GET['export_csv'])) {
    $rows = $pdo->prepare("SELECT * FROM commissary_cogs WHERE store_name='Commissary' AND cogs_year=? AND cogs_month=? ORDER BY cogs_date ASC");
    $rows->execute([$fYear,$fMonth]);
    $data = $rows->fetchAll();

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="commissary_cogs_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['Commissary Branch — COGS']);
    ...
    // Totals row
    $tot = $pdo->prepare("SELECT COALESCE(SUM(beg),0) beg, COALESCE(SUM(purc),0) purc, COALESCE(SUM(end_inv),0) end_inv, COALESCE(SUM(cos),0) cos, COALESCE(SUM(mktg_cost),0) mktg_cost FROM commissary_cogs WHERE store_name='Commissary' AND cogs_year=? AND cogs_month=?");
...
// Sum all total_amount entries from commissary_month_end_inv for any day in selected month/year
$meiStmt = $pdo->prepare("SELECT COALESCE(SUM(total_amount),0) AS grand_total FROM commissary_month_end_inv WHERE store_name='Commissary' AND inv_year=? AND inv_month=?");
$meiStmt->execute([$fYear,$fMonth]);
$meiTotal = (float)$meiStmt->fetchColumn();

// ── Load rows for selected month ──────────────────────────
$stmt = $pdo->prepare("SELECT * FROM commissary_cogs WHERE store_name='Commissary' AND cogs_year=? AND cogs_month=? ORDER BY cogs_date ASC");
$stmt->execute([$fYear,$fMonth]);
$rows = $stmt->fetchAll();

// ── Monthly totals ────────────────────────────────────────
$totStmt = $pdo->prepare("SELECT COALESCE(SUM(beg),0) beg, COALESCE(SUM(purc),0) purc, COALESCE(SUM(end_inv),0) end_inv, COALESCE(SUM(cos),0) cos, COALESCE(SUM(mktg_cost),0) mktg_cost FROM commissary_cogs WHERE store_name='Commissary' AND cogs_year=? AND cogs_month=?");
...
<div class="cogs-header">
  <div class="cogs-header-left">
    <div class="eyebrow">Commissary Branch · COGS</div>
...
<div class="cogs-controls">
  ...
  <a href="commissary_cogs.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
</div>
...
async function saveRow(btn) {
  ...
  try {
    const res  = await fetch('commissary_cogs.php', {method:'POST', body:fd});
...
async function deleteRow(btn) {
  ...
  if (id > 0) {
    ...
    fd.append('row_id', id);
    try {
      await fetch('commissary_cogs.php', {method:'POST', body:fd});
    } catch(e) {}
  }
