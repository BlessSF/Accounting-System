<?php
// ============================================================
//  commissary_sum.php — Commissary Sum / Checking View
//  Mirrors the Excel "Checking" tab format:
//  ASSETS + EXPENSES rows × Jan–Dec months with TOTAL column
//  Data pulled live from commissary_disbursement
// ============================================================
...
if (isset($_GET['export_csv'])) {
    ...
    header('Content-Disposition: attachment; filename="commissary_sum_'.$YEAR.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['Commissary — Sum / Checking', $YEAR]);
    fputcsv($out, []);
    ...
    $sections = [
        'ASSETS' => [
            ...
            'Leasehold Improvement','Furniture and Fixtures','Investments',
        ],
        'EXPENSES' => [
            'Purchases - Non-Vat','Purchases - Vatable','Kitchen Supplies',
            'Solane','Miscellaneous','Rent','CUSA','Office Supplies',
            'Pest Control','Advertisement','Bio Augmentation','Professional Fee',
            'Bookkeeping Fee','Fare & Transportation','Fuel & Oil',
            'Repairs and Maintenance','Telephone, Light & Water','Delivery Expense',
            'Salaries and Wages','Representation Expense','Meals',
            'Taxes and Licenses','SSS, PHIC, HDMF Contribution',
            'Commission Expense',"M'Nikki",'c/o Nikki','Others',
        ],
    ];
...
$pageTitle  = 'Commissary Sum';
$activePage = 'commissary_disbursement'; // keep Disbursement active in sidebar
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title">Commissary <span>Sum</span></div>
...
<div class="sum-controls">
  ...
  <a href="?year=<?= $YEAR+1 ?>" class="yr-btn"><?= $YEAR+1 ?> →</a>
  <a href="commissary_disbursement.php" class="btn-action">📒 Go to Disbursement</a>
