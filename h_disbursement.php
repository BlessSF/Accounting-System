<?php
// ============================================================
//  h_disbursement.php — H Disbursement Journal
...
// Only H branch + management can access
...
// ── Auto-create table ──────────────────────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `h_disbursement` (
...
// ── Account Titles (managed via h_acc_title.php) ───────────
...
if ((int)$pdo->query("SELECT COUNT(*) FROM `h_acc_titles`")->fetchColumn() === 0) {
    $DEFAULT_TITLES = [
    ...
    $ins = $pdo->prepare("INSERT IGNORE INTO `h_acc_titles` (`title`,`section`,`sort_order`,`saved_by`) VALUES (?,?,?,?)");
    $i = 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        $sql = "INSERT INTO h_disbursement (" . implode(',', array_map(fn($f)=>"`$f`", $fields)) . ") VALUES (" . implode(',', array_fill(0, count($fields), '?')) . ")";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $vals[] = $id;
    try {
        $pdo->prepare("UPDATE h_disbursement SET " . implode(',', $sets) . " WHERE id=?")->execute($vals);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    try {
        $pdo->prepare("DELETE FROM h_disbursement WHERE id=?")->execute([$id]);
        echo json_encode(['ok' => true]);
    } catch (Throwable $e) {
        echo json_encode(['ok' => false, 'msg' => $e->getMessage()]);
    }
...
if (isset($_GET['export_csv'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM h_disbursement $where ORDER BY entry_date ASC, id ASC");
    $stmt->execute($params);
    $rows = $stmt->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="H_Disbursement_' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['MERITONI CORP']);
    fputcsv($out, ['H Disbursement Journal']);
    ...
    // Totals
    $stmt2 = $pdo->prepare("SELECT SUM(gross) g, SUM(input_tax) t, SUM(net_of_vat) n FROM h_disbursement $where");
...
$stmt = $pdo->prepare("SELECT * FROM h_disbursement $whereSQL ORDER BY entry_date ASC, id ASC");
$stmt->execute($params);
$rows = $stmt->fetchAll();

// Totals
$stmtT = $pdo->prepare("SELECT SUM(gross) g, SUM(input_tax) t, SUM(net_of_vat) n FROM h_disbursement $whereSQL");
$stmtT->execute($params);
$totals = $stmtT->fetch();

$acctTitles = $pdo->query("SELECT title FROM `h_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_COLUMN);

$pageTitle  = 'H Disbursement';
...
<form method="GET" class="dsb-controls" id="filterForm">
  ...
  <?php if ($fSearch): ?>
  <a href="h_disbursement.php?month=<?= urlencode($fMonth) ?>" class="btn btn-ghost btn-sm">Reset</a>
  <?php endif; ?>
  <button type="button" class="btn-add-row" onclick="addRow()">+ Add Entry</button>
  <a href="h_disbursement.php?export_csv=1&month=<?= urlencode($fMonth) ?>"
     class="btn btn-ghost btn-sm" style="color:#388E3C;border-color:rgba(76,175,80,.3);background:rgba(76,175,80,.06)">
    ⬇ Download CSV
  </a>
  <a href="h_profit_loss.php" class="btn btn-ghost btn-sm" style="color:#1565C0;border-color:rgba(21,101,192,.3);background:rgba(21,101,192,.06);font-weight:600">
    📊 Profit &amp; Loss
  </a>
  <a href="h_sum.php?year=<?= date('Y') ?>" class="btn btn-ghost btn-sm" style="color:#5D4037;border-color:rgba(249,168,37,.4);background:rgba(249,168,37,.08);font-weight:600">
    📋 Sum
  </a>
  <a href="h_tb.php?year=<?= date('Y') ?>" class="btn btn-ghost btn-sm" style="color:#37474F;border-color:rgba(55,71,79,.3);background:rgba(55,71,79,.06);font-weight:600">
    🗂 TB
  </a>
  <a href="h_acc_title.php" class="btn btn-ghost btn-sm" style="color:#6D4C41;border-color:rgba(109,76,65,.3);background:rgba(109,76,65,.06);font-weight:600">
    🏷 Account Titles
  </a>
...
function renderRow(array $r, array $acctOpts): string {
    // Account titles now come from the h_acc_titles table
    // (managed via h_acc_title.php) instead of being hardcoded here.
...
<script>
// Account titles now come from h_acc_titles (manage via h_acc_title.php)
...
async function doAC(inp) {
    ...
    try {
        const res  = await fetch('h_disbursement.php?vendor_lookup=1&q=' + encodeURIComponent(q));
...
async function saveRow(btn) {
    ...
    try {
        const res  = await fetch('h_disbursement.php', { method: 'POST', body: fd });
        const data = await res.json();
        if (data.ok) {
