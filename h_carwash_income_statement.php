<?php
// ============================================================
//  h_carwash_income_statement.php — Hero Carwash Income Statement
//  Mirrors h_income_statement.php's structure exactly, adapted to
//  Carwash's own tables. No expenses ledger exists for Carwash, so
//  VAT Payment is a manual entry here instead of auto-pulled, and
//  the Expenses Ledger Summary card is dropped.
// ============================================================
...
// ── Auto-create Carwash Income Statement table (safe to run repeatedly) ──
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS `h_carwash_income_statement` (
        ...
        PRIMARY KEY (`id`),
        UNIQUE KEY `unique_date_store` (`stmt_date`, `store_name`),
        KEY `idx_date` (`stmt_date`)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    $fields = [
        ...
        // Tax — manual entry here (no expenses ledger exists for Carwash)
        'vat_payment',
    ];

    $data = [
        'store_name'   => CIS_STORE,
    ...
    try {
        $sql = "INSERT INTO h_carwash_income_statement ("
...
$row = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM h_carwash_income_statement WHERE store_name=? AND stmt_date=? LIMIT 1");
    $stmt->execute([CIS_STORE, $stmtDate]);
    $row = $stmt->fetch() ?: null;
} catch (Throwable $ignored) {}

$v = fn($k) => $row ? number_format((float)($row[$k] ?? 0), 2, '.', '') : '0.00';
// ── Auto-pull gross total from Carwash Summary Report ─────
// Net Sales = SUM(store_gross_excl_mktg) for the selected month
$expYear  = (int)date('Y', strtotime($stmtDate));
$expMonth = (int)date('n', strtotime($stmtDate));
$storeGrossTotal = 0;
try {
    $sgStmt = $pdo->prepare("SELECT COALESCE(SUM(`store_gross_excl_mktg`),0) FROM `h_carwash_summary_entries` WHERE store_name=? AND YEAR(report_date)=? AND MONTH(report_date)=?");
    $sgStmt->execute([CIS_STORE, $expYear, $expMonth]);
...
try {
    ...
    $expStmt = $pdo->prepare("SELECT $selectParts FROM `h_expenses` WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
$pageTitle  = 'Carwash Income Statement';
$activePage = 'h_carwash_income_statement';
include 'layout.php';
?>
...
<div class="is-wrap" style="max-width:100%;margin-bottom:20px">
  ...
  <div class="is-card" style="overflow-x:auto">
    <div style="background:#1e3a5f;padding:12px 22px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-family:var(--font-m);font-size:.58rem;text-transform:uppercase;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-bottom:2px">H Branch</div>
      ...
      <a href="h_expenses.php?month=<?= $expMonth ?>&year=<?= $expYear ?>"
...
<div class="is-card">
  ...
  <!-- Header -->
  <div class="is-header">
    <div class="is-header-title">H Branch</div>
...
async function saveAll() {
  ...
  try {
    const res  = await fetch('h_income_statement.php', {method:'POST', body:fd});
