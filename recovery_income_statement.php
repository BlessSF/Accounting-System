<?php
// ============================================================
//  recovery_income_statement.php — Recovery Hero Income Statement
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        $sql = "INSERT INTO recovery_income_statement ("
...
$row = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM recovery_income_statement WHERE store_name='Recovery' AND stmt_date=? LIMIT 1");
...
// Net Sales = SUM(pos_reading) for the selected month
$expYear  = (int)date('Y', strtotime($stmtDate));
$expMonth = (int)date('n', strtotime($stmtDate));
$storeGrossTotal = 0;
try {
    $sgStmt = $pdo->prepare("SELECT COALESCE(SUM(`pos_reading`),0) FROM `recovery_report_entries` WHERE store_name='Recovery' AND YEAR(report_date)=? AND MONTH(report_date)=?");
...
try {
    ...
    $expStmt = $pdo->prepare("SELECT $selectParts FROM `recovery_expenses` WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
$activePage = 'recovery_income_statement';
include 'layout.php';
?>
...
<div class="is-wrap" style="max-width:100%;margin-bottom:20px">
  ...
  <div class="is-card" style="overflow-x:auto">
    <div style="background:#1e3a5f;padding:12px 22px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-family:var(--font-m);font-size:.58rem;text-transform:uppercase;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-bottom:2px">Recovery Branch</div>
      ...
      <a href="recovery_expenses.php?month=<?= $expMonth ?>&year=<?= $expYear ?>"
...
<div class="is-card">
  ...
  <!-- Header -->
  <div class="is-header">
    <div class="is-header-title">Recovery Branch</div>
  ...
  <div class="is-body">
    ...
    ?>
    <!-- Net Sales — auto-pulled from Branch Summary Report POS Reading total -->
    <div class="is-row">
      <div class="is-label">
        Net Sales
...
async function saveAll() {
  ...
  try {
    const res  = await fetch('recovery_income_statement.php', {method:'POST', body:fd});
