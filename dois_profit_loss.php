<?php
// ============================================================
//  dois_profit_loss.php — Dois Profit & Loss Statement
//  Pulls expense data from dois_disbursement table
...
$pdo  = getPDO();
// Safety guard — dois_acc_titles is normally created by
// dois_disbursement.php / dois_acc_title.php, but ensure it exists
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `dois_pl_revenue` (
    `id`         int(11) NOT NULL AUTO_INCREMENT,
    `year`       int(4)  NOT NULL,
...
// These now come from dois_acc_titles (managed via dois_acc_title.php)
...
$EXPENSE_ACCOUNTS = $pdo->query("SELECT title FROM `dois_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_rev'])) {
    ...
    try {
        $pdo->prepare("INSERT INTO dois_pl_revenue (year,month,rev_type,amount,saved_by)
                       VALUES (?,?,?,?,?)
...
$revRows = $pdo->prepare("SELECT month, rev_type, amount FROM dois_pl_revenue WHERE year=? ORDER BY month,rev_type");
...
$pageTitle  = 'Dois Profit & Loss';
$activePage = 'dois_profit_loss';
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title">Dois <span>Profit &amp; Loss</span></div>
...
<div class="pl-controls">
  ...
  <a href="?year=<?= $YEAR+1 ?>" class="yr-btn"><?= $YEAR+1 ?> →</a>
  <a href="dois_disbursement.php" class="btn-export">📒 Go to Disbursement</a>
  <a href="dois_sum.php?year=<?= $YEAR ?>" class="btn-export" style="background:#FFF9C4;border-color:#F9A825;color:#5D4037;font-weight:700">📋 Sum</a>
  <a href="dois_acc_title.php" class="btn-export" style="background:#EFEBE9;border-color:#6D4C41;color:#4E342E;font-weight:700">🏷 Account Titles</a>
...
async function saveRev(inp) {
    ...
    try {
        const res  = await fetch('dois_profit_loss.php', { method:'POST', body:fd });
...
function exportCSV() {
    const rows = [];
    rows.push(['Dois Profit & Loss — ' + YEAR]);
