<?php
// ============================================================
//  demiclab_reconcile.php — DemicLab-Main Branch Bank Reconciliation
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_reconcile` (
...
foreach (['ending_balance_books', 'adjusted_bank_balance', 'adjusted_book_balance'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `demiclab_reconcile` ADD COLUMN `$newCol` decimal(12,2) DEFAULT NULL");
...
function pullBooksEndingBalance(PDO $pdo, string $store, int $year, int $month): array {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM demiclab_cashflow_balance
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        [$endBooks, ] = pullBooksEndingBalance($pdo, 'DemicLab-Main', $fYear, $fMonth);
        $adjBank   = $endBank + $depTrans - $outChecks;
        $adjBook   = $endBooks + $bankCred - $bankChg;
        $variance  = $adjBook - $adjBank;

        $pdo->prepare("
            INSERT INTO demiclab_reconcile (store_name, rec_year, rec_month, ending_balance_bank, deposits_in_transit, outstanding_checks, ending_balance_books, bank_credits, bank_charges, adjusted_bank_balance, adjusted_book_balance, saved_by)
            VALUES ('DemicLab-Main', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
if (isset($_GET['export_csv'])) {
    $stmt = $pdo->prepare("SELECT * FROM demiclab_reconcile WHERE store_name='DemicLab-Main' AND rec_year=? AND rec_month=?");
    ...
    [$pulledBooks, ] = pullBooksEndingBalance($pdo, 'DemicLab-Main', $fYear, $fMonth);
    ...
    header('Content-Disposition: attachment; filename="DemicLab-Main_Reconcile_'.$months[$fMonth-1].'_'.$fYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['BANK RECONCILIATION']);
    fputcsv($out, ['DemicLab-Main Branch', $months[$fMonth-1].' '.$fYear]);
    fputcsv($out, []);
...
[$pulledBooks, $booksDebug] = pullBooksEndingBalance($pdo, 'DemicLab-Main', $fYear, $fMonth);
...
$pageTitle  = 'DemicLab-Main Reconcile';
$activePage = 'demiclab_reconcile';
include 'layout.php';
?>
...
<div class="rc-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="demiclab_reconcile.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveReconcile() {
  ...
  try {
    const res  = await fetch('demiclab_reconcile.php?year='+R_YEAR+'&month='+R_MONTH, {method:'POST', body:fd});
