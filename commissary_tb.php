<?php
// ============================================================
//  commissary_tb.php — Commissary Trial Balance
...
// ── Section definitions (matching Excel screenshot exactly) ──
$ASSETS = [
    'Office Equipment',
    'Other Equipment',
    'Service Vehicle',
...
if (isset($_GET['export_csv'])) {
    ...
    header('Content-Disposition: attachment; filename="Commissary_TB_'.$YEAR.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['MERITONI CORP — Commissary Trial Balance', $YEAR]);
    fputcsv($out, []);
...
$pageTitle  = 'Commissary TB';
$activePage = 'commissary_disbursement';
include 'layout.php';
?>
...
<div class="section-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:20px">
  <div>
    <div class="section-title">Commissary <span>Trial Balance</span></div>
    <div class="section-subtitle">MERITONI CORP · Assets, Revenue &amp; Expenses by month · <?= $YEAR ?></div>
  </div>
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    <a href="commissary_disbursement.php" class="nav-btn btn-dsb">📒 Disbursement</a>
    <a href="commissary_sum.php?year=<?= $YEAR ?>" class="nav-btn btn-sum">📋 Sum</a>
  </div>
</div>
