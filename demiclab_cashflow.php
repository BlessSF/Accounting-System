<?php
// ============================================================
//  demiclab_cashflow.php — Stella Branch Cashflow Statement
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_cashflow` (
...
// ── Helper: live-pull SUM(pdc) from demiclab_expenses with full diagnostics ──
function pullPdcTotal(PDO $pdo, int $year, int $month): array {
    ...
    try {
        ...
        $chk = $pdo->query("SHOW TABLES LIKE 'demiclab_expenses'");
        $debug['table_exists'] = $chk && $chk->rowCount() > 0;

        if (!$debug['table_exists']) {
            $debug['error'] = "Table 'demiclab_expenses' does not exist on this database.";
            return [0.0, $debug];
        }

        // 2. Does the pdc column exist?
        $colChk = $pdo->query("SHOW COLUMNS FROM demiclab_expenses LIKE 'pdc'");
        $debug['column_exists'] = $colChk && $colChk->rowCount() > 0;

        if (!$debug['column_exists']) {
            $debug['error'] = "Column 'pdc' does not exist in demiclab_expenses. Run the PDC/CA/Withdrawal/Depreciation migration SQL.";
            return [0.0, $debug];
        }

        // 3. Pull the actual sum
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(pdc),0) AS total_pdc FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        // Inventory Purchases — always re-pulled live: SUM(purchases) from demiclab_expenses + 18.75
        $invPurc = 0.0;
        try {
            $purcStmt = $pdo->prepare("SELECT COALESCE(SUM(purchases),0) FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
            $purcStmt->execute([$fYear,$fMonth]);
            $invPurc = (float)$purcStmt->fetchColumn() + 18.75;
        } catch (Throwable $e) {
            $invPurc = 18.75;
        }
        // Withdrawals (Pull-out) — always re-pulled live: SUM(withdrawal) from demiclab_expenses
        $withdrawals = 0.0;
        try {
            $wdStmt = $pdo->prepare("SELECT COALESCE(SUM(withdrawal),0) FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
        ...
        try {
            ...
            ),0) AS total_expenses FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
        ...
        $sgStmt = $pdo->prepare("SELECT COALESCE(SUM(store_gross),0) FROM summary_report_entries WHERE store_name='DemicLab-Main' AND YEAR(report_date)=? AND MONTH(report_date)=?");
        ...
        $existing = $pdo->prepare("SELECT id FROM demiclab_cashflow WHERE store_name='DemicLab-Main' AND cf_year=? AND cf_month=? LIMIT 1");
        $existing->execute([$fYear,$fMonth]);
        $existingId = $existing->fetchColumn();

        if ($existingId) {
            $pdo->prepare("UPDATE demiclab_cashflow SET cf_date=?, cash_beg=?, sales=?, inv_purchases=?, expenses=?, pdc_loan=?, withdrawals=?, net_cash_flow=?, cash_end=?, saved_by=? WHERE id=?")
                ->execute([$lastDay,$cashBeg,$sales,$invPurc,$expenses,$pdcLoan,$withdrawals,$netCashFlow,$cashEnd,$user['name'],$existingId]);
        } else {
            $pdo->prepare("INSERT INTO demiclab_cashflow (cf_date,cf_year,cf_month,store_name,cash_beg,sales,inv_purchases,expenses,pdc_loan,withdrawals,net_cash_flow,cash_end,saved_by) VALUES (?,?,?,'DemicLab-Main',?,?,?,?,?,?,?,?,?)")
...
if (isset($_GET['export_csv'])) {
    $s = $pdo->prepare("SELECT * FROM demiclab_cashflow WHERE store_name='DemicLab-Main' AND cf_year=? AND cf_month=? LIMIT 1");
    $s->execute([$fYear,$fMonth]);
    $row = $s->fetch() ?: null;
    $cv  = fn($k) => $row ? number_format((float)($row[$k] ?? 0),2,'.','') : '0.00';

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="demiclab_cashflow_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
...
// Inventory Purchases — always live-pulled: SUM(purchases) from demiclab_expenses + 18.75
$invPurc = 0.0;
try {
    $purcStmt = $pdo->prepare("SELECT COALESCE(SUM(purchases),0) FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
    $purcStmt->execute([$fYear,$fMonth]);
...
// Expenses — always live-pulled: sum of all operating expense columns from demiclab_expenses
$expenses = 0.0;
try {
    $expStmt = $pdo->prepare("SELECT COALESCE(SUM(
        salaries + rent + medicine + lpg + repairs_maintenance + fuel_trans +
...
// Withdrawals (Pull-out) — always live-pulled: SUM(withdrawal) from demiclab_expenses
$withdrawals = 0.0;
try {
    $wdStmt = $pdo->prepare("SELECT COALESCE(SUM(withdrawal),0) FROM demiclab_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
    $wdStmt->execute([$fYear,$fMonth]);
...
<div class="cf-header-card">
  <div>
    <div class="eyebrow">DemicLab-Main Branch · Finance</div>
...
<div class="cf-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="demiclab_cashflow.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
<div class="cf-card">
  ...
  <div class="cf-row indent">
    <div class="cf-label">
      Inventory Purchases
      <span class="auto-badge">↳ Auto from Expenses + 18.75</span>
    </div>
    <div class="cf-input-wrap">
...
async function saveCashflow() {
  ...
  try {
    const res  = await fetch('demiclab_cashflow.php', {method:'POST', body:fd});
