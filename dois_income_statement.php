<?php
// ============================================================
//  dois_income_statement.php — Dois Hero Income Statement
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        $sql = "INSERT INTO dois_income_statement ("
...
$row = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM dois_income_statement WHERE store_name='Dois' AND stmt_date=? LIMIT 1");
    $stmt->execute([$stmtDate]);
    $row = $stmt->fetch() ?: null;
} catch (Throwable $ignored) {}

$v = fn($k) => $row ? number_format((float)($row[$k] ?? 0), 2, '.', '') : '0.00';
// ── Auto-pull gross total from Branch Summary Report ──────
// Net Sales = SUM(gross_sales) for the selected month.
// This mirrors the Summary Report page's own logic: for each date, the
// Sales Report's Gross Sales wins if present (even if that Summary row
// hasn't been clicked "Save" yet); otherwise fall back to whatever was
// saved in dois_report_entries. This keeps this total identical to the
// TOTAL shown at the bottom of the Summary Report, regardless of which
// individual day-rows have been manually saved.
$expYear  = (int)date('Y', strtotime($stmtDate));
$expMonth = (int)date('n', strtotime($stmtDate));
$storeGrossTotal = 0;
try {
    $savedGross = [];
    $sgStmt = $pdo->prepare("SELECT report_date, gross_sales FROM dois_report_entries WHERE store_name='Dois' AND YEAR(report_date)=? AND MONTH(report_date)=?");
    $sgStmt->execute([$expYear, $expMonth]);
    foreach ($sgStmt->fetchAll(PDO::FETCH_ASSOC) as $r) $savedGross[$r['report_date']] = (float)$r['gross_sales'];

    $srGross = [];
    $srStmt = $pdo->prepare("SELECT report_date, gross_sales FROM dois_sales_report WHERE store_name='Dois' AND YEAR(report_date)=? AND MONTH(report_date)=?");
    $srStmt->execute([$expYear, $expMonth]);
    foreach ($srStmt->fetchAll(PDO::FETCH_ASSOC) as $r) $srGross[$r['report_date']] = (float)$r['gross_sales'];

    $allDates = array_unique(array_merge(array_keys($savedGross), array_keys($srGross)));
    foreach ($allDates as $d) {
        if (isset($srGross[$d]) && $srGross[$d] != 0) {
            $storeGrossTotal += $srGross[$d];
        } elseif (isset($savedGross[$d])) {
            $storeGrossTotal += $savedGross[$d];
        }
    }
} catch (Throwable $ignored) {}
$netSalesAuto = number_format($storeGrossTotal, 2, '.', '');
// ── Expenses column totals (for the selected month) ──────
$expCols = [
...
try {
    ...
    $expStmt = $pdo->prepare("SELECT $selectParts FROM `dois_expenses` WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
$activePage = 'dois_income_statement';
include 'layout.php';
?>
...
<div class="is-wrap" style="max-width:100%;margin-bottom:20px">
  ...
  <div class="is-card" style="overflow-x:auto">
    <div style="background:#1e3a5f;padding:12px 22px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-family:var(--font-m);font-size:.58rem;text-transform:uppercase;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-bottom:2px">Dois Branch</div>
      ...
      <a href="dois_expenses.php?month=<?= $expMonth ?>&year=<?= $expYear ?>"
...
<div class="is-card">
  ...
  <!-- Header -->
  <div class="is-header">
    <div class="is-header-title">Dois Branch</div>
  ...
  <div class="is-body">
    ...
    ?>
    <!-- Net Sales — auto-pulled from Branch Summary Report store_gross total -->
    <div class="is-row">
      <div class="is-label">
        Net Sales
...
async function saveAll() {
  ...
  try {
    const res  = await fetch('dois_income_statement.php', {method:'POST', body:fd});
