<?php
// ============================================================
//  dois_reconcile.php — Dois Branch Bank Reconciliation
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `dois_reconcile` (
...
foreach (['ending_balance_books', 'adjusted_bank_balance', 'adjusted_book_balance'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `dois_reconcile` ADD COLUMN `$newCol` decimal(12,2) DEFAULT NULL");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        [$endBooks, ] = pullBooksEndingBalance($pdo, 'Dois', $fYear, $fMonth);
        $adjBank   = $endBank + $depTrans - $outChecks;
        $adjBook   = $endBooks + $bankCred - $bankChg;
        $variance  = $adjBook - $adjBank;

        $pdo->prepare("
            INSERT INTO dois_reconcile (store_name, rec_year, rec_month, ending_balance_bank, deposits_in_transit, outstanding_checks, ending_balance_books, bank_credits, bank_charges, adjusted_bank_balance, adjusted_book_balance, saved_by)
            VALUES ('Dois', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
if (isset($_GET['export_csv'])) {
    $stmt = $pdo->prepare("SELECT * FROM dois_reconcile WHERE store_name='Dois' AND rec_year=? AND rec_month=?");
    ...
    [$pulledBooks, ] = pullBooksEndingBalance($pdo, 'Dois', $fYear, $fMonth);
    ...
    header('Content-Disposition: attachment; filename="Dois_Reconcile_'.$months[$fMonth-1].'_'.$fYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['BANK RECONCILIATION']);
    fputcsv($out, ['Dois Branch', $months[$fMonth-1].' '.$fYear]);
    fputcsv($out, []);
...
[$pulledBooks, $booksDebug] = pullBooksEndingBalance($pdo, 'Dois', $fYear, $fMonth);
...
$pageTitle  = 'Dois Reconcile';
$activePage = 'dois_reconcile';
include 'layout.php';
?>
...
<div class="rc-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="dois_reconcile.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveReconcile() {
  ...
  try {
    const res  = await fetch('dois_reconcile.php?year='+R_YEAR+'&month='+R_MONTH, {method:'POST', body:fd});
