<?php
// ============================================================
//  pub_express_profit_loss.php — Pub Express Profit & Loss Statement
//  Pulls expense data from pub_express_disbursement table
...
$pdo  = getPDO();
// Safety guard — pub_express_acc_titles is normally created by
// pub_express_disbursement.php / pub_express_acc_title.php, but ensure it
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `pub_express_pl_revenue` (
    `id`         int(11) NOT NULL AUTO_INCREMENT,
    `year`       int(4)  NOT NULL,
...
// These now come from pub_express_acc_titles (managed via pub_express_acc_title.php)
// instead of a hardcoded list — P&L lists every account title regardless of
// section (Assets purchases are shown as spend here too, same as before).
$EXPENSE_ACCOUNTS = $pdo->query("SELECT title FROM `pub_express_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_COLUMN);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_rev'])) {
    ...
    try {
        $pdo->prepare("INSERT INTO pub_express_pl_revenue (year,month,rev_type,amount,saved_by)
                       VALUES (?,?,?,?,?)
...
$revRows = $pdo->prepare("SELECT month, rev_type, amount FROM pub_express_pl_revenue WHERE year=? ORDER BY month,rev_type");
...
$pageTitle  = 'Pub Express Profit & Loss';
$activePage = 'pub_express_profit_loss';
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title">Pub Express <span>Profit &amp; Loss</span></div>
...
<div class="pl-controls">
  ...
  <a href="?year=<?= $YEAR+1 ?>" class="yr-btn"><?= $YEAR+1 ?> →</a>
  <a href="pub_express_disbursement.php" class="btn-export">📒 Go to Disbursement</a>
  <a href="pub_express_sum.php?year=<?= $YEAR ?>" class="btn-export" style="background:#FFF9C4;border-color:#F9A825;color:#5D4037;font-weight:700">📋 Sum</a>
  <a href="pub_express_acc_title.php" class="btn-export" style="background:#EFEBE9;border-color:#6D4C41;color:#4E342E;font-weight:700">🏷 Account Titles</a>
...
async function saveRev(inp) {
    ...
    try {
        const res  = await fetch('pub_express_profit_loss.php', { method:'POST', body:fd });
...
function exportCSV() {
    const rows = [];
    rows.push(['Pub Express Profit & Loss — ' + YEAR]);
    ...
    a.download = 'Pub_Express_PL_' + YEAR + '.csv';
    a.click();
}
