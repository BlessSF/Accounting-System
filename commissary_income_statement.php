<?php
// ============================================================
//  commissary_income_statement.php — Commissary Hero Income Statement
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        $sql = "INSERT INTO commissary_income_statement ("
...
$row = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM commissary_income_statement WHERE store_name='Commissary' AND stmt_date=? LIMIT 1");
...
try {
    ...
    $expStmt = $pdo->prepare("SELECT $selectParts FROM `commissary_expenses` WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
$activePage = 'commissary_income_statement';
include 'layout.php';
?>
...
<div class="is-wrap" style="max-width:100%;margin-bottom:20px">
  ...
  <div class="is-card" style="overflow-x:auto">
    <div style="background:#1e3a5f;padding:12px 22px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
      <div>
        <div style="font-family:var(--font-m);font-size:.58rem;text-transform:uppercase;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-bottom:2px">Commissary Branch</div>
      ...
      <a href="commissary_expenses.php?month=<?= $expMonth ?>&year=<?= $expYear ?>"
...
<div class="is-card">
  ...
  <!-- Header -->
  <div class="is-header">
    <div class="is-header-title">Commissary Branch</div>
...
async function saveAll() {
  ...
  try {
    const res  = await fetch('commissary_income_statement.php', {method:'POST', body:fd});
