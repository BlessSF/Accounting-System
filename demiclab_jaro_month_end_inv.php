<?php
// ============================================================
//  demiclab_jaro_month_end_inv.php — DemicLab-Jaro Branch Month End Inventory
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_jaro_month_end_inv` (
...
// New columns for the DEPARTMENT / Quantity / Unit Price / Price per Piece /
// Start Inv.# / Consumed layout — added on top of the original schema so
// existing saved data (supplier_cost, end_inv_num, total_amount) is never lost.
foreach ([
    ...
    try { $pdo->exec("ALTER TABLE `demiclab_jaro_month_end_inv` ADD COLUMN $col"); }
    catch (Throwable $ignored) {}
}

// ── Sections (custom categories) table ─────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_jaro_categories` (
    `id`          int(11) NOT NULL AUTO_INCREMENT,
    `store_name`  varchar(50) NOT NULL DEFAULT 'DemicLab-Jaro',
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_jaro_categories_meta` (
...
$alreadySeeded = (bool)$pdo->query("SELECT seeded FROM demiclab_jaro_categories_meta WHERE store_name='DemicLab-Jaro'")->fetchColumn();
if (!$alreadySeeded) {
    $catCount = (int)$pdo->query("SELECT COUNT(*) FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro'")->fetchColumn();
    if ($catCount === 0) {
        $defaults = ['LIQUORS/BEVERAGES','DRY GOODS','BAR STOCKS','MEAT/WET PRODUCTS',
                     'VEGETABLES','PASTA','CONDIMENTS','BREAD'];
        $insDef = $pdo->prepare("INSERT IGNORE INTO demiclab_jaro_categories (store_name,name,sort_order) VALUES ('DemicLab-Jaro',?,?)");
        foreach ($defaults as $i => $name) $insDef->execute([$name, $i]);
    }
    $pdo->prepare("INSERT INTO demiclab_jaro_categories_meta (store_name,seeded) VALUES ('DemicLab-Jaro',1) ON DUPLICATE KEY UPDATE seeded=1")->execute();
}

// One-time self-heal: any category name already used inside
// demiclab_jaro_month_end_inv (e.g. from a renamed/typo'd section) that isn't
// registered yet gets auto-registered, so its items don't disappear.
$knownNames = $pdo->query("SELECT name FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro'")->fetchAll(PDO::FETCH_COLUMN);
$usedNames  = $pdo->query("SELECT DISTINCT category FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND category<>''")->fetchAll(PDO::FETCH_COLUMN);
$missing    = array_diff($usedNames, $knownNames);
if (!empty($missing)) {
    $maxOrd = (int)$pdo->query("SELECT COALESCE(MAX(sort_order),-1) FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro'")->fetchColumn();
    $insMiss = $pdo->prepare("INSERT IGNORE INTO demiclab_jaro_categories (store_name,name,sort_order) VALUES ('DemicLab-Jaro',?,?)");
...
$catStmt = $pdo->query("SELECT name FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro' ORDER BY sort_order,id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        $pricePiece  = (float)($_POST['price_per_piece'] ?? 0);
        $startInv    = (float)($_POST['start_inv_num']   ?? 0);
        $endInv      = (float)($_POST['end_inv_num']     ?? 0);
        $remarks     = trim($_POST['remarks']    ?? '');
        $additional  = trim($_POST['additional'] ?? '');

        // Calculated fields — same formulas as the client-side preview,
        // recomputed here so the stored values can never drift out of sync.
        $pricePieceStart = round($startInv * $pricePiece, 2);   // Price per Piece (Start Inventory)
        $consumed        = round($startInv - $endInv, 4);       // Consumed
        $total           = round($endInv * $pricePiece, 2);     // Total Amount

        $yr = (int)date('Y', strtotime($lastDay));
        $mo = (int)date('n', strtotime($lastDay));

        if ($rowId > 0) {
            $pdo->prepare("UPDATE demiclab_jaro_month_end_inv SET inv_date=?, inv_year=?, inv_month=?, category=?, sort_order=?, item_desc=?, unit=?, quantity=?, unit_price=?, price_per_piece=?, start_inv_num=?, price_per_piece_start=?, consumed=?, end_inv_num=?, total_amount=?, remarks=?, additional=?, saved_by=? WHERE id=? AND store_name='DemicLab-Jaro'")
                ->execute([$lastDay,$yr,$mo,$category,$sortOrder,$itemDesc,$unit,$quantity,$unitPrice,$pricePiece,$startInv,$pricePieceStart,$consumed,$endInv,$total,$remarks,$additional,$user['name'],$rowId]);
            echo json_encode(['ok'=>true,'id'=>$rowId,'total'=>$total,'price_per_piece_start'=>$pricePieceStart,'consumed'=>$consumed]);
        } else {
            $pdo->prepare("INSERT INTO demiclab_jaro_month_end_inv (inv_date,inv_year,inv_month,store_name,category,sort_order,item_desc,unit,quantity,unit_price,price_per_piece,start_inv_num,price_per_piece_start,consumed,end_inv_num,total_amount,remarks,additional,saved_by) VALUES (?,?,?,'DemicLab-Jaro',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
                ->execute([$lastDay,$yr,$mo,$category,$sortOrder,$itemDesc,$unit,$quantity,$unitPrice,$pricePiece,$startInv,$pricePieceStart,$consumed,$endInv,$total,$remarks,$additional,$user['name']]);
            $newId = (int)$pdo->lastInsertId();
            echo json_encode(['ok'=>true,'id'=>$newId,'total'=>$total,'price_per_piece_start'=>$pricePieceStart,'consumed'=>$consumed]);
        }
    } catch (Throwable $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM demiclab_jaro_month_end_inv WHERE id=? AND store_name='DemicLab-Jaro'")->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add_category'])) {
    ...
    try {
        ...
        $exists = $pdo->prepare("SELECT id FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro' AND name=?");
        $exists->execute([$name]);
        if ($exists->fetch()) { echo json_encode(['ok'=>false,'msg'=>'A section named "'.$name.'" already exists.']); exit; }
        $maxOrd = (int)$pdo->query("SELECT COALESCE(MAX(sort_order),-1) FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro'")->fetchColumn();
        $pdo->prepare("INSERT INTO demiclab_jaro_categories (store_name,name,sort_order) VALUES ('DemicLab-Jaro',?,?)")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete_category'])) {
    ...
    try {
        ...
        $chk   = $pdo->prepare("SELECT COUNT(*) FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND category=?");
        $chk->execute([$name]);
        ...
        if ($itemCount > 0 && $force) {
            $pdo->prepare("DELETE FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND category=?")->execute([$name]);
        }
        $pdo->prepare("DELETE FROM demiclab_jaro_categories WHERE store_name='DemicLab-Jaro' AND name=?")->execute([$name]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_duplicate_day'])) {
    ...
    try {
        ...
        $srcStmt = $pdo->prepare("SELECT * FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_date=? ORDER BY sort_order,id");
        ...
        $chkStmt = $pdo->prepare("SELECT COUNT(*) FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_date=?");
        $delStmt = $pdo->prepare("DELETE FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_date=?");
        $insStmt = $pdo->prepare("INSERT INTO demiclab_jaro_month_end_inv (inv_date,inv_year,inv_month,store_name,category,sort_order,item_desc,unit,quantity,unit_price,price_per_piece,start_inv_num,price_per_piece_start,consumed,end_inv_num,total_amount,remarks,additional,saved_by) VALUES (?,?,?,'DemicLab-Jaro',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
        ...
        foreach ($targets as $tDate) {
            ...
            foreach ($srcRows as $r) {
                // Description-level fields always carry over; the count/qty
                // fields (and everything computed from them) only copy across
                // when "Also copy quantities & totals" is checked.
                $unitPrice  = (float)$r['unit_price'];
                $pricePiece = (float)$r['price_per_piece'];
                $startInv   = $copyQty ? (float)$r['start_inv_num'] : 0;
                $endInv     = $copyQty ? (float)$r['end_inv_num']   : 0;
                $pricePieceStart = $copyQty ? round($startInv * $pricePiece, 2) : 0;
                $consumed        = $copyQty ? round($startInv - $endInv, 4)     : 0;
                $total           = $copyQty ? round($endInv * $pricePiece, 2)   : 0;
                $insStmt->execute([$tDate,$yr,$mo,$r['category'],$r['sort_order'],$r['item_desc'],$r['unit'],$r['quantity'],$unitPrice,$pricePiece,$startInv,$pricePieceStart,$consumed,$endInv,$total,$r['remarks'],$r['additional'],$user['name']]);
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_month_counts'])) {
    ...
    try {
        ...
        $cStmt = $pdo->prepare("SELECT DAY(inv_date) as d, COUNT(*) as c FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_year=? AND inv_month=? GROUP BY DAY(inv_date)");
        $cStmt->execute([$yr, $mo]);
...
if (isset($_GET['export_csv'])) {
    $stmt = $pdo->prepare("SELECT * FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_date=? ORDER BY sort_order, id");
    $stmt->execute([$selectedDate]);
    $rows = $stmt->fetchAll();

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="demiclab_jaro_month_end_inv_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['DATE: ' . $displayDate]);
    fputcsv($out,['DEPARTMENT','ItemDescription','Unit','Quantity','Unit Price','Price per Piece','Start Inv. #','Price per Piece (Start Inventory)','CONSUMED','End. Inv. Number','Total Amount','Remarks','Additional']);
    ...
    foreach ($CATEGORIES as $cat) {
        ...
        foreach ($catRows as $r) {
            fputcsv($out,[
                $cat, $r['item_desc'], $r['unit'], $r['quantity'],
                number_format($r['unit_price'],2,'.',''),
                number_format($r['price_per_piece'],4,'.',''),
                number_format($r['start_inv_num'],4,'.',''),
                number_format($r['price_per_piece_start'],2,'.',''),
                number_format($r['consumed'],4,'.',''),
                number_format($r['end_inv_num'],4,'.',''),
                number_format($r['total_amount'],2,'.',''),
                $r['remarks'], $r['additional'],
            ]);
            $catTotal += (float)$r['total_amount'];
        }
        fputcsv($out,array_merge(['','','','','','','','','','TOTAL — '.$cat, number_format($catTotal,2,'.','')]));
        $grandTotal += $catTotal;
        fputcsv($out,['']);
    }
    fputcsv($out,array_merge(['','','','','','','','','','GRAND TOTAL', number_format($grandTotal,2,'.','')]));
    fclose($out);
    exit;
}

// ── Load rows ─────────────────────────────────────────────
// Display helper: show up to 4 decimals but trim trailing zeros
// (2.2000 -> 2.2, 500.0000 -> 500, 1100.00 -> 1100). Fixes DB
// decimal(12,4) columns printing padded zeros in the inputs.
function nf(int|float|string $v): string {
    $v = (float)$v;
    if ($v == 0) return '';
    $s = number_format($v, 4, '.', '');
    $s = rtrim(rtrim($s, '0'), '.');
...
$dayCountStmt = $pdo->prepare("SELECT DAY(inv_date) as d, COUNT(*) as c FROM demiclab_jaro_month_end_inv WHERE store_name='DemicLab-Jaro' AND inv_year=? AND inv_month=? GROUP BY DAY(inv_date)");
$dayCountStmt->execute([$fYear, $fMonth]);
$dayCounts = array_column($dayCountStmt->fetchAll(PDO::FETCH_ASSOC), 'c', 'd');

$pageTitle  = 'Month End Inv.';
$activePage = 'demiclab_jaro_month_end_inv';
include 'layout.php';
?>
...
.inv-scroll-wrap { overflow-x: auto; }
.inv-col-head, .inv-row, .inv-total-row { min-width: 1580px; }
...
.inv-col-head {
  display: grid;
  grid-template-columns: 110px minmax(180px,1.4fr) 80px 110px 95px 100px 85px 130px 90px 100px 110px 130px 110px 100px;
  gap: 0;
  background: #1e3a5f;
...
.inv-col-head div:first-child,
.inv-col-head div:nth-child(2) { text-align: left; padding-left: 14px; }
...
.inv-input.calc-input {
  background: #f8f9fb; color: #4b5563;
  font-weight: 600; cursor: default;
  text-align: right;
}
.dept-cell {
  padding: 7px 10px; font-family: var(--font-m); font-size: .74rem;
...
.inv-row:hover .inv-action-cell { background: #f8fbff; }
.inv-col-head > div:last-child {
...
.inv-total-row div:last-child   { border-right: none; position: sticky; right: 0; }
...
<div class="inv-header">
  <div>
    <div class="eyebrow">DemicLab-Jaro Branch · Inventory</div>
    <div class="title">Month End Inventory</div>
    <div class="subtitle">DATE: <?= $displayDate ?> &nbsp;·&nbsp; Total auto-calculates from Price per Piece × End. Inv. Number</div>
  </div>
...
<div class="inv-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="demiclab_jaro_month_end_inv.php?export_csv=1&month=<?=$fMonth?>&day=<?=$fDay?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
<div class="inv-section" id="section_<?= $catIdx ?>" data-cat="<?= htmlspecialchars($cat) ?>">
  ...
  <div class="inv-col-head">
    ...
    <div>Start Inv. #</div>
    <div>Price per Piece (Start Inventory)</div>
    <div>Consumed</div>
    <div>End. Inv. Number</div>
  ...
  <div id="tbody_<?= $catIdx ?>">
    <?php if ($catRows): ?>
      ...
      <div class="inv-row" data-id="<?= $r['id'] ?>" data-cat="<?= $catIdx ?>">
        <div class="inv-cell dept-cell"><?= htmlspecialchars($cat) ?></div>
        <div class="inv-cell">
        ...
        <div class="inv-cell">
          <input type="text" class="inv-input qty-input" value="<?= htmlspecialchars($r['quantity']) ?>" placeholder="e.g. 100pcs/box" oninput="markChanged(this)">
        </div>
        <div class="inv-cell">
          <input type="number" step="0.01" class="inv-input num-input"
                 value="<?= nf($r['unit_price']) ?>" placeholder="0.00"
                 oninput="calcRow(this)">
        </div>
        <div class="inv-cell">
          <input type="number" step="0.01" class="inv-input num-input"
                 value="<?= nf($r['price_per_piece']) ?>" placeholder="0.00"
                 oninput="calcRow(this)">
        </div>
        <div class="inv-cell">
          <input type="number" step="1" class="inv-input num-input"
                 value="<?= nf($r['start_inv_num']) ?>" placeholder="0"
                 oninput="calcRow(this)">
        </div>
        <div class="inv-cell">
          <input type="number" step="0.01" class="inv-input calc-input"
                 value="<?= nf($r['price_per_piece_start']) ?>" placeholder="0.00" readonly tabindex="-1">
        </div>
        <div class="inv-cell">
          <input type="number" step="1" class="inv-input calc-input"
                 value="<?= nf($r['consumed']) ?>" placeholder="0" readonly tabindex="-1">
        </div>
        <div class="inv-cell">
          <input type="number" step="1" class="inv-input num-input"
                 value="<?= nf($r['end_inv_num']) ?>" placeholder="0"
                 oninput="calcRow(this)">
        </div>
        <div class="inv-cell">
          <input type="number" step="0.01" class="inv-input total-input"
                 value="<?= nf($r['total_amount']) ?>" placeholder="0.00" readonly tabindex="-1">
        </div>
        <div class="inv-cell">
          <input type="text" class="inv-input remarks-input" value="<?= htmlspecialchars($r['remarks'] ?? '') ?>" placeholder="Remarks…" oninput="markChanged(this)">
        </div>
        <div class="inv-cell">
          <input type="text" class="inv-input additional-input" value="<?= htmlspecialchars($r['additional'] ?? '') ?>" placeholder="Additional…" oninput="markChanged(this)">
        </div>
...
// Mirrors the PHP nf() helper: show up to 4 decimals but trim trailing
// zeros (2.20 -> 2.2, 500.0000 -> 500), so calculated fields never show
// more decimal noise than the number actually has.
function trimNum(n) {
  if (!n) return '';
  let s = n.toFixed(4);
...
function calcRow(el) {
  ...
  // .num-input order in the row: [0]=Unit Price [1]=Price per Piece [2]=Start Inv.# [3]=End Inv. Number
  const unitPrice  = gv(inputs[0]);
  const pricePiece = gv(inputs[1]);
  const startInv   = gv(inputs[2]);
  const endInv     = gv(inputs[3]);

  const pricePieceStart = Math.round(startInv * pricePiece * 100) / 100;   // Price per Piece (Start Inventory)
  const consumed        = Math.round((startInv - endInv) * 10000) / 10000; // Consumed
  const total           = Math.round(endInv * pricePiece * 100) / 100;     // Total Amount

  const calcInputs = row.querySelectorAll('.calc-input');
  calcInputs[0].value = trimNum(pricePieceStart);
  calcInputs[1].value = trimNum(consumed);
  row.querySelector('.total-input').value = trimNum(total);
...
function addRow(catIdx, catName) {
  ...
  div.innerHTML = `
    <div class="inv-cell dept-cell">${catName.replace(/</g,'&lt;')}</div>
    <div class="inv-cell">
    ...
    <div class="inv-cell">
      <input type="text" class="inv-input qty-input" placeholder="e.g. 100pcs/box" oninput="markChanged(this)">
    </div>
    <div class="inv-cell">
      <input type="number" step="0.01" class="inv-input num-input" value="" placeholder="0.00" oninput="calcRow(this)">
    </div>
    <div class="inv-cell">
      <input type="number" step="0.01" class="inv-input num-input" value="" placeholder="0.00" oninput="calcRow(this)">
    </div>
    <div class="inv-cell">
      <input type="number" step="1" class="inv-input num-input" value="" placeholder="0" oninput="calcRow(this)">
    </div>
    <div class="inv-cell">
      <input type="number" step="0.01" class="inv-input calc-input" value="" placeholder="0.00" readonly tabindex="-1">
    </div>
    <div class="inv-cell">
      <input type="number" step="1" class="inv-input calc-input" value="" placeholder="0" readonly tabindex="-1">
    </div>
    <div class="inv-cell">
      <input type="number" step="1" class="inv-input num-input" value="" placeholder="0" oninput="calcRow(this)">
    </div>
    ...
    <div class="inv-cell">
      <input type="text" class="inv-input remarks-input" placeholder="Remarks…" oninput="markChanged(this)">
    </div>
    <div class="inv-cell">
      <input type="text" class="inv-input additional-input" placeholder="Additional…" oninput="markChanged(this)">
    </div>
    <div class="inv-cell inv-action-cell">
      <div style="display:flex;flex-direction:column;align-items:center;gap:2px">
        <div style="display:flex;gap:3px">
...
async function saveRow(btn, catName, catIdx) {
  ...
  const additional  = row.querySelector('.additional-input').value.trim();
  ...
  fd.append('price_per_piece', pricePiece);
  fd.append('start_inv_num',   startInv);
  fd.append('end_inv_num',     endInv);
  fd.append('remarks',         remarks);
  fd.append('additional',      additional);

  try {
    const res  = await fetch('demiclab_jaro_month_end_inv.php', {method:'POST', body:fd});
    const json = await res.json();
    if (json.ok) {
      row.dataset.id = json.id;
      // Update calculated fields from the server (source of truth)
      const calcInputs = row.querySelectorAll('.calc-input');
      calcInputs[0].value = trimNum(parseFloat(json.price_per_piece_start));
      calcInputs[1].value = trimNum(parseFloat(json.consumed));
      row.querySelector('.total-input').value = trimNum(parseFloat(json.total));
...
async function deleteRow(btn, catIdx) {
  ...
  if (id > 0) {
    ...
    fd.append('row_id', id);
    try { await fetch('demiclab_jaro_month_end_inv.php', {method:'POST', body:fd}); } catch(e) {}
  }
