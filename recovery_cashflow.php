<?php
// ============================================================
//  recovery_cashflow.php — Recovery Branch Cashflow Statement
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_cashflow` (
...
// ── Helper: live-pull SUM(pdc) from recovery_expenses with full diagnostics ──
function pullPdcTotal(PDO $pdo, int $year, int $month): array {
    ...
    try {
        ...
        $chk = $pdo->query("SHOW TABLES LIKE 'recovery_expenses'");
        $debug['table_exists'] = $chk && $chk->rowCount() > 0;

        if (!$debug['table_exists']) {
            $debug['error'] = "Table 'recovery_expenses' does not exist on this database.";
            return [0.0, $debug];
        }

        // 2. Does the pdc column exist?
        $colChk = $pdo->query("SHOW COLUMNS FROM recovery_expenses LIKE 'pdc'");
        $debug['column_exists'] = $colChk && $colChk->rowCount() > 0;

        if (!$debug['column_exists']) {
            $debug['error'] = "Column 'pdc' does not exist in recovery_expenses. Run the PDC/CA/Withdrawal/Depreciation migration SQL.";
            return [0.0, $debug];
        }

        // 3. Pull the actual sum
        $stmt = $pdo->prepare("SELECT COALESCE(SUM(pdc),0) AS total_pdc FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
function pullSelectedVatTotal(PDO $pdo, string $store, int $year, int $month): float {
    try {
        $pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_cf_vat_selection` (
            `id`            int(11) NOT NULL AUTO_INCREMENT,
            `store_name`    varchar(50) NOT NULL DEFAULT 'Recovery',
        ...
        $stmt = $pdo->prepare("SELECT vat_total FROM recovery_cf_vat_selection WHERE store_name=? AND sel_year=? AND sel_month=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        // Inventory Purchases — always re-pulled live: SUM(purchases) from recovery_expenses
        // + whatever VAT total was saved from the checkbox-selected rows on the Expenses page
        $invPurc = 0.0;
        try {
            $purcStmt = $pdo->prepare("SELECT COALESCE(SUM(purchases),0) FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
            $purcStmt->execute([$fYear,$fMonth]);
            $selectedVat = pullSelectedVatTotal($pdo, 'Recovery', $fYear, $fMonth);
            $invPurc = (float)$purcStmt->fetchColumn() + $selectedVat;
        } catch (Throwable $e) {
            $invPurc = pullSelectedVatTotal($pdo, 'Recovery', $fYear, $fMonth);
        }
        // Withdrawals (Pull-out) — always re-pulled live: SUM(withdrawal) from recovery_expenses
        $withdrawals = 0.0;
        try {
            $wdStmt = $pdo->prepare("SELECT COALESCE(SUM(withdrawal),0) FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
        ...
        try {
            ...
            ),0) AS total_expenses FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
        ...
        $sgStmt = $pdo->prepare("SELECT COALESCE(SUM(store_gross),0) FROM summary_report_entries WHERE store_name='Recovery' AND YEAR(report_date)=? AND MONTH(report_date)=?");
        ...
        $existing = $pdo->prepare("SELECT id FROM recovery_cashflow WHERE store_name='Recovery' AND cf_year=? AND cf_month=? LIMIT 1");
        $existing->execute([$fYear,$fMonth]);
        $existingId = $existing->fetchColumn();

        if ($existingId) {
            $pdo->prepare("UPDATE recovery_cashflow SET cf_date=?, cash_beg=?, sales=?, inv_purchases=?, expenses=?, pdc_loan=?, withdrawals=?, net_cash_flow=?, cash_end=?, saved_by=? WHERE id=?")
                ->execute([$lastDay,$cashBeg,$sales,$invPurc,$expenses,$pdcLoan,$withdrawals,$netCashFlow,$cashEnd,$user['name'],$existingId]);
        } else {
            $pdo->prepare("INSERT INTO recovery_cashflow (cf_date,cf_year,cf_month,store_name,cash_beg,sales,inv_purchases,expenses,pdc_loan,withdrawals,net_cash_flow,cash_end,saved_by) VALUES (?,?,?,'Recovery',?,?,?,?,?,?,?,?,?)")
...
if (isset($_GET['export_csv'])) {
    $s = $pdo->prepare("SELECT * FROM recovery_cashflow WHERE store_name='Recovery' AND cf_year=? AND cf_month=? LIMIT 1");
    $s->execute([$fYear,$fMonth]);
    $row = $s->fetch() ?: null;
    $cv  = fn($k) => $row ? number_format((float)($row[$k] ?? 0),2,'.','') : '0.00';

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="recovery_cashflow_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
...
// Inventory Purchases — always live-pulled: SUM(purchases) from recovery_expenses
// + whatever VAT total was saved from the checkbox-selected rows on the Expenses page
$invPurc = 0.0;
$selectedVatTotal = pullSelectedVatTotal($pdo, 'Recovery', $fYear, $fMonth);
try {
    $purcStmt = $pdo->prepare("SELECT COALESCE(SUM(purchases),0) FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
    $purcStmt->execute([$fYear,$fMonth]);
...
// Expenses — always live-pulled: sum of all operating expense columns from recovery_expenses
$expenses = 0.0;
try {
    $expStmt = $pdo->prepare("SELECT COALESCE(SUM(
        salaries + rent + medicine + lpg + repairs_maintenance + fuel_trans +
...
// Withdrawals (Pull-out) — always live-pulled: SUM(withdrawal) from recovery_expenses
$withdrawals = 0.0;
try {
    $wdStmt = $pdo->prepare("SELECT COALESCE(SUM(withdrawal),0) FROM recovery_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
    $wdStmt->execute([$fYear,$fMonth]);
...
<div class="cf-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="recovery_cashflow.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveCashflow() {
  ...
  try {
    const res  = await fetch('recovery_cashflow.php', {method:'POST', body:fd});
