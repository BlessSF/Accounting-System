<?php
// recovery_cashflow_balance.php — Recovery Branch Cashflow Balance (Cash In / Cash Out / Running Balance)
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_cashflow_balance` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        ...
        $pdo->prepare("INSERT INTO recovery_cashflow_balance (store_name,txn_date,description,cash_in,cash_out,entry_year,entry_month,created_by) VALUES ('Recovery',?,?,?,?,?,?,?)")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    try {
        ...
        $in   = (float)($_POST['cash_in']  ?? 0);
        $out  = (float)($_POST['cash_out'] ?? 0);
        $pdo->prepare("UPDATE recovery_cashflow_balance SET txn_date=?,description=?,cash_in=?,cash_out=? WHERE id=? AND store_name='Recovery'")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        $id = (int)$_POST['id'];
        $pdo->prepare("DELETE FROM recovery_cashflow_balance WHERE id=? AND store_name='Recovery'")
            ->execute([$id]);
        echo json_encode(['ok'=>true]);
    } catch (Throwable $e) {
        echo json_encode(['ok'=>false,'msg'=>$e->getMessage()]);
...
if (isset($_GET['export_csv'])) {
    $rows = $pdo->prepare("SELECT * FROM recovery_cashflow_balance WHERE store_name='Recovery' AND entry_year=? AND entry_month=? ORDER BY txn_date ASC, id ASC");
    $rows->execute([$fYear,$fMonth]);
    $rows = $rows->fetchAll(PDO::FETCH_ASSOC);
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="Recovery_CashflowBalance_'.$monthNames[$fMonth].'_'.$fYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['Recovery Branch — Cashflow Balance']);
...
$stmt = $pdo->prepare("SELECT * FROM recovery_cashflow_balance WHERE store_name='Recovery' AND entry_year=? AND entry_month=? ORDER BY txn_date ASC, id ASC");
...
$activePage = 'recovery_cashflow_balance';
include 'layout.php';
?>
...
<div class="cb-wrap">
  ...
  <div class="cb-controls">
    ...
    <a href="recovery_cashflow_balance.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost btn-sm" style="color:var(--accent3);border-color:rgba(251,191,36,.25);background:rgba(251,191,36,.06)">⬇ Download CSV</a>
  </div>
...
async function saveRow(btn) {
  ...
  try {
    const res  = await fetch('recovery_cashflow_balance.php', {method:'POST', body:fd});
