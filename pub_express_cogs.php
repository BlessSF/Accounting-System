<?php
// ============================================================
//  pub_express_cogs.php — Pub Express Branch COGS (Cost of Goods Sold)
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `pub_express_cogs` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        if ($rowId > 0) {
            $pdo->prepare("UPDATE pub_express_cogs SET cogs_date=?, cogs_year=?, cogs_month=?, beg=?, purc=?, end_inv=?, cos=?, mktg_cost=?, saved_by=? WHERE id=? AND store_name='Pub Express'")
                ->execute([$date,$yr,$mo,$beg,$purc,$end,$cos,$mktg,$user['name'],$rowId]);
            echo json_encode(['ok'=>true, 'id'=>$rowId, 'cos'=>number_format($cos,2)]);
        } else {
            $sql = "INSERT INTO pub_express_cogs (cogs_date, cogs_year, cogs_month, store_name, beg, purc, end_inv, cos, mktg_cost, saved_by) VALUES (?,?,?,'Pub Express',?,?,?,?,?,?)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM pub_express_cogs WHERE id=? AND store_name='Pub Express'")->execute([$id]);
...
if (isset($_GET['export_csv'])) {
    $rows = $pdo->prepare("SELECT * FROM pub_express_cogs WHERE store_name='Pub Express' AND cogs_year=? AND cogs_month=? ORDER BY cogs_date ASC");
    $rows->execute([$fYear,$fMonth]);
    $data = $rows->fetchAll();

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="pub_express_cogs_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['Pub Express Branch — COGS']);
    ...
    // Totals row
    $tot = $pdo->prepare("SELECT COALESCE(SUM(beg),0) beg, COALESCE(SUM(purc),0) purc, COALESCE(SUM(end_inv),0) end_inv, COALESCE(SUM(cos),0) cos, COALESCE(SUM(mktg_cost),0) mktg_cost FROM pub_express_cogs WHERE store_name='Pub Express' AND cogs_year=? AND cogs_month=?");
...
// Sum all total_amount entries from pub_express_month_end_inv for any day in selected month/year
$meiStmt = $pdo->prepare("SELECT COALESCE(SUM(total_amount),0) AS grand_total FROM pub_express_month_end_inv WHERE store_name='Pub Express' AND inv_year=? AND inv_month=?");
$meiStmt->execute([$fYear,$fMonth]);
$meiTotal = (float)$meiStmt->fetchColumn();

// ── Load rows for selected month ──────────────────────────
$stmt = $pdo->prepare("SELECT * FROM pub_express_cogs WHERE store_name='Pub Express' AND cogs_year=? AND cogs_month=? ORDER BY cogs_date ASC");
$stmt->execute([$fYear,$fMonth]);
$rows = $stmt->fetchAll();

// ── Monthly totals ────────────────────────────────────────
$totStmt = $pdo->prepare("SELECT COALESCE(SUM(beg),0) beg, COALESCE(SUM(purc),0) purc, COALESCE(SUM(end_inv),0) end_inv, COALESCE(SUM(cos),0) cos, COALESCE(SUM(mktg_cost),0) mktg_cost FROM pub_express_cogs WHERE store_name='Pub Express' AND cogs_year=? AND cogs_month=?");
...
<div class="cogs-header">
  <div class="cogs-header-left">
    <div class="eyebrow">Pub Express Branch · COGS</div>
...
<div class="cogs-controls">
  ...
  <a href="pub_express_cogs.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
</div>
...
async function saveRow(btn) {
  ...
  try {
    const res  = await fetch('pub_express_cogs.php', {method:'POST', body:fd});
...
async function deleteRow(btn) {
  ...
  if (id > 0) {
    ...
    fd.append('row_id', id);
    try {
      await fetch('pub_express_cogs.php', {method:'POST', body:fd});
    } catch(e) {}
  }
