<?php
// ============================================================
//  demiclab_sales_report.php — DemicLab-Main Branch Daily Sales Report
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_sales_report` (
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_dinein_rows` (
    `id`            int(11) NOT NULL AUTO_INCREMENT,
    `store_name`    varchar(50) NOT NULL DEFAULT 'DemicLab-Main',
    `report_date`   date NOT NULL,
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_sales_detail_rows` (
    `id`            int(11) NOT NULL AUTO_INCREMENT,
    `store_name`    varchar(50) NOT NULL DEFAULT 'DemicLab-Main',
    `report_date`   date NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        $diSumStmt = $pdo->prepare("SELECT
            ...
            FROM demiclab_dinein_rows WHERE store_name='DemicLab-Main' AND report_date=?");
        ...
        $secSumStmt = $pdo->prepare("SELECT section, COALESCE(SUM(amount),0) AS total
            FROM demiclab_sales_detail_rows WHERE store_name='DemicLab-Main' AND report_date=? GROUP BY section");
        ...
        $data      = array_merge(['DemicLab-Main', $reportDate], array_values($vals), [$netSales, $shortOver, $user['name']]);
        $dupUpdate = implode(',', array_map(fn($f) => "`$f`=VALUES(`$f`)", array_merge($NUM_COLS, ['net_sales','short_over','saved_by'])));
        $sql = "INSERT INTO demiclab_sales_report (" . implode(',', array_map(fn($f)=>"`$f`",$fields)) . ")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_dinein'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM demiclab_dinein_rows WHERE store_name='DemicLab-Main' AND report_date=?")->execute([$reportDate]);
        $ins = $pdo->prepare("INSERT INTO demiclab_dinein_rows (store_name,report_date,cash,palawan_pay,card_swipe_qr,unpaid_credit_name,unpaid_credit_amount,discount,bank_transfer_cheque,cancelled_transactions,sort_order) VALUES ('DemicLab-Main',?,?,?,?,?,?,?,?,?,?)");
        foreach ($rows as $i => $r) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_detail'])) {
    ...
    try {
        ...
        $rows = json_decode($_POST['rows'] ?? '[]', true);
        $pdo->prepare("DELETE FROM demiclab_sales_detail_rows WHERE store_name='DemicLab-Main' AND report_date=? AND section=?")->execute([$reportDate,$section]);
        $ins = $pdo->prepare("INSERT INTO demiclab_sales_detail_rows (store_name,report_date,section,item_name,amount,sort_order) VALUES ('DemicLab-Main',?,?,?,?,?)");
        foreach ($rows as $i => $r) {
...
if (isset($_GET['export_csv'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM demiclab_sales_report WHERE store_name='DemicLab-Main' AND report_date=?");
    $stmt->execute([$reportDate]);
    $r = $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
    $g = fn($k) => number_format((float)($r[$k] ?? 0), 2, '.', '');
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="DemicLab-Main_SalesReport_'.$reportDate.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['SUMMARY REPORT — DemicLab-Main Branch', date('F j, Y', strtotime($reportDate))]);
    fputcsv($out, []);
...
// Fetch DINE IN rows
$diStmt = $pdo->prepare("SELECT * FROM demiclab_dinein_rows WHERE store_name='DemicLab-Main' AND report_date=? ORDER BY sort_order ASC");
...
$detailStmt = $pdo->prepare("SELECT * FROM demiclab_sales_detail_rows WHERE store_name='DemicLab-Main' AND report_date=? ORDER BY section, sort_order ASC");
...
$pageTitle  = 'DemicLab-Main Sales Report';
$activePage = 'demiclab_sales_report';
include 'layout.php';
?>
...
<div class="sr-header-card">
  <div>
    <div class="eyebrow">DemicLab-Main Branch · Sales</div>
  ...
  <span style="background:rgba(255,255,255,.1);color:#fff;padding:5px 14px;border-radius:20px;font-family:var(--font-m);font-size:.65rem;font-weight:600;align-self:flex-start">📌 DemicLab-Main</span>
</div>
...
<div class="sr-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="demiclab_sales_report.php?export_csv=1&date=<?= htmlspecialchars($fDate) ?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveAll() {
  ...
  await fetch('demiclab_sales_report.php', {method:'POST',body:fd1});
  ...
  for (const sec of allSecs) {
    ...
    await fetch('demiclab_sales_report.php', {method:'POST',body:fd2});
  }
  ...
  const res  = await fetch('demiclab_sales_report.php', {method:'POST',body:fd3});
