<?php
// ============================================================
//  demiclab_jaro_expenses.php — DemicLab-Jaro Branch Expenses Ledger
...
// Only DemicLab-Jaro branch and management can access
...
// ── Auto-create table ─────────────────────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `demiclab_jaro_expenses` (
...
foreach (['pdc','ca','withdrawal','depreciation_expense'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `demiclab_jaro_expenses` ADD COLUMN `$newCol` decimal(12,2) DEFAULT 0.00 AFTER `sales_discounts`");
...
foreach (['pest_control_bio_aug','commission_fees','exhaust_cleaning','bank_fees','admin_salary_shares'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `demiclab_jaro_expenses` ADD COLUMN `$newCol` decimal(12,2) DEFAULT 0.00 AFTER `staff_meal`");
    } catch (Throwable $ignored) {} // column already exists — ignore
}
...
try {
    $pdo->exec("ALTER TABLE `demiclab_jaro_expenses` ADD COLUMN `selected_for_cf` tinyint(1) NOT NULL DEFAULT 0 AFTER `row_total`");
} catch (Throwable $ignored) {} // column already exists — ignore
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        $sql = "INSERT INTO demiclab_jaro_expenses (" . implode(',', array_map(fn($f)=>"`$f`",$fields)) . ") VALUES (" . implode(',', array_fill(0,count($fields),'?')) . ")";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $vals[] = $id;
    try {
        $pdo->prepare("UPDATE demiclab_jaro_expenses SET ".implode(',',$sets)." WHERE id=?")->execute($vals);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    try {
        $pdo->prepare("DELETE FROM demiclab_jaro_expenses WHERE id=?")->execute([$id]);
        echo json_encode(['ok'=>true]);
    } catch (Throwable $e) {
        echo json_encode(['ok'=>false,'msg'=>$e->getMessage()]);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_selected_vat'])) {
    ...
    try {
        $pdo->prepare("
            INSERT INTO demiclab_jaro_cf_vat_selection (store_name, sel_year, sel_month, vat_total, row_count, saved_by)
            VALUES ('DemicLab-Jaro', ?, ?, ?, ?, ?)
...
if (isset($_GET['export_csv'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM demiclab_jaro_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=? ORDER BY expense_date ASC, id ASC");
    $stmt->execute([$csvYear, $csvMonth]);
    $rows = $stmt->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="DemicLab-Jaro_Expenses_'.$monthNames[$csvMonth].'_'.$csvYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['DemicLab-Jaro Branch — Expenses Ledger']);
...
try {
    ...
    $totStmt = $pdo->prepare("SELECT $selectParts FROM demiclab_jaro_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
try {
    $svStmt = $pdo->prepare("SELECT vat_total, row_count, saved_by FROM demiclab_jaro_cf_vat_selection WHERE store_name='DemicLab-Jaro' AND sel_year=? AND sel_month=?");
...
$pageTitle  = 'DemicLab-Jaro Expenses';
$activePage = 'demiclab_jaro_expenses';
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title" style="color:#1a1d23">DemicLab-Jaro Branch <span style="color:var(--accent)">Expenses Ledger</span></div>
...
async function saveSelectedVat(silent) {
  ...
  try {
    const res  = await fetch('demiclab_jaro_expenses.php', {method:'POST', body:fd});
...
function doExport(e) {
  ...
  window.location.href = 'demiclab_jaro_expenses.php?' + params.toString();
}
...
async function doAC(inp) {
  ...
  try {
    const res  = await fetch('demiclab_jaro_expenses.php?vendor_lookup=1&q=' + encodeURIComponent(q));
    const data = await res.json();
    if (!data.length) {
