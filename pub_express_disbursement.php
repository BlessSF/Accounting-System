<?php
// ============================================================
//  pub_express_disbursement.php — Pub Express Disbursement Journal
...
// Only Pub Express branch + management can access
...
// ── Auto-create table ──────────────────────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `pub_express_disbursement` (
...
// ── Account Titles (managed via pub_express_acc_title.php) ─
$pdo->exec("CREATE TABLE IF NOT EXISTS `pub_express_acc_titles` (
    `id`          int(11) NOT NULL AUTO_INCREMENT,
...
try { $pdo->exec("ALTER TABLE `pub_express_acc_titles` ADD COLUMN `section` enum('assets','expenses','other') NOT NULL DEFAULT 'expenses' AFTER `title`"); }
catch (Throwable $ignored) {}

// One-time seed from the list that used to be hardcoded in
// pub_express_disbursement.php / pub_express_sum.php / pub_express_tb.php,
// so existing entries and report groupings keep working.
if ((int)$pdo->query("SELECT COUNT(*) FROM `pub_express_acc_titles`")->fetchColumn() === 0) {
    $DEFAULT_TITLES = [
    ...
    $ins = $pdo->prepare("INSERT IGNORE INTO `pub_express_acc_titles` (`title`,`section`,`sort_order`,`saved_by`) VALUES (?,?,?,?)");
    $i = 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        $sql = "INSERT INTO pub_express_disbursement (" . implode(',', array_map(fn($f)=>"`$f`", $fields)) . ") VALUES (" . implode(',', array_fill(0, count($fields), '?')) . ")";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $vals[] = $id;
    try {
        $pdo->prepare("UPDATE pub_express_disbursement SET " . implode(',', $sets) . " WHERE id=?")->execute($vals);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    try {
        $pdo->prepare("DELETE FROM pub_express_disbursement WHERE id=?")->execute([$id]);
        echo json_encode(['ok' => true]);
    } catch (Throwable $e) {
        echo json_encode(['ok' => false, 'msg' => $e->getMessage()]);
    }
...
if (isset($_GET['export_csv'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM pub_express_disbursement $where ORDER BY entry_date ASC, id ASC");
    $stmt->execute($params);
    $rows = $stmt->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="Pub_Express_Disbursement_' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['MERITONI CORP']);
    fputcsv($out, ['Pub Express Disbursement Journal']);
    ...
    // Totals
    $stmt2 = $pdo->prepare("SELECT SUM(gross) g, SUM(input_tax) t, SUM(net_of_vat) n FROM pub_express_disbursement $where");
...
$stmt = $pdo->prepare("SELECT * FROM pub_express_disbursement $whereSQL ORDER BY entry_date ASC, id ASC");
$stmt->execute($params);
$rows = $stmt->fetchAll();

// Totals
$stmtT = $pdo->prepare("SELECT SUM(gross) g, SUM(input_tax) t, SUM(net_of_vat) n FROM pub_express_disbursement $whereSQL");
$stmtT->execute($params);
$totals = $stmtT->fetch();

$acctTitles = $pdo->query("SELECT title FROM `pub_express_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_COLUMN);

$pageTitle  = 'Pub Express Disbursement';
$activePage = 'pub_express_disbursement';
include 'layout.php';
?>
...
<div class="section-header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
  ...
  <a href="pub_express_profit_loss.php"
     style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;
  ...
  <a href="pub_express_sum.php?year=<?= date('Y') ?>"
     style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;
  ...
  <a href="pub_express_tb.php?year=<?= date('Y') ?>"
     style="display:inline-flex;align-items:center;gap:7px;padding:9px 18px;
...
<form method="GET" class="dsb-controls" id="filterForm">
  ...
  <?php if ($fSearch): ?>
  <a href="pub_express_disbursement.php?month=<?= urlencode($fMonth) ?>" class="btn btn-ghost btn-sm">Reset</a>
  <?php endif; ?>
  <button type="button" class="btn-add-row" onclick="addRow()">+ Add Entry</button>
  <a href="pub_express_disbursement.php?export_csv=1&month=<?= urlencode($fMonth) ?>"
     class="btn btn-ghost btn-sm" style="color:#388E3C;border-color:rgba(76,175,80,.3);background:rgba(76,175,80,.06)">
    ⬇ Download CSV
  </a>
  <a href="pub_express_acc_title.php" class="btn btn-ghost btn-sm" style="color:#6D4C41;border-color:rgba(109,76,65,.3);background:rgba(109,76,65,.06);font-weight:600">
    🏷 Account Titles
  </a>
  <a href="pub_express_profit_loss.php" class="btn btn-ghost btn-sm" style="color:#1565C0;border-color:rgba(21,101,192,.3);background:rgba(21,101,192,.06);font-weight:600">
    📊 Profit &amp; Loss
  </a>
  <a href="pub_express_sum.php?year=<?= date('Y') ?>" class="btn btn-ghost btn-sm" style="color:#5D4037;border-color:rgba(249,168,37,.4);background:rgba(249,168,37,.08);font-weight:600">
    📋 Sum
  </a>
  <a href="pub_express_tb.php?year=<?= date('Y') ?>" class="btn btn-ghost btn-sm" style="color:#37474F;border-color:rgba(55,71,79,.3);background:rgba(55,71,79,.06);font-weight:600">
    🗂 TB
  </a>
...
function renderRow(array $r, array $acctOpts): string {
    // Account titles now come from the pub_express_acc_titles table
    // (managed via pub_express_acc_title.php) instead of being hardcoded here.
...
<script>
// Account titles now come from pub_express_acc_titles (manage via pub_express_acc_title.php)
...
// ── Autocomplete ───────────────────────────────────────────
// The dropdown list is reparented to <body> and positioned with
// position:fixed so the scrolling table container (.dsb-outer) never
// clips it — instead of living inside .ac-wrap where overflow:auto/hidden
// on an ancestor would cut it off.
...
function openAC(inp, list) {
    positionAC(inp, list);
    list.classList.add('open');
}

function closeAC(list) {
    list.classList.remove('open');
}

function closeAllAC() {
    document.querySelectorAll('.ac-list').forEach(closeAC);
}

async function doAC(inp) {
    const q = inp.value.trim();
    const list = inp.closest('.ac-wrap').querySelector('.ac-list');
    if (q.length < 2) { closeAC(list); return; }
    try {
        const res  = await fetch('pub_express_disbursement.php?vendor_lookup=1&q=' + encodeURIComponent(q));
...
function fillVendor(item) {
    const list = item.closest('.ac-list');
    const inp  = list._ownerInput;
...
// Close dropdowns when clicking outside (the list now lives in <body>,
// so we also check .ac-list itself, not just .ac-wrap)
document.addEventListener('click', e => {
    if (!e.target.closest('.ac-wrap') && !e.target.closest('.ac-list')) closeAllAC();
});

// Keep open dropdowns aligned with their input while the table scrolls
// (horizontal scroll on .dsb-outer, or the page itself) or the window resizes
document.addEventListener('scroll', () => {
    document.querySelectorAll('.ac-list.open').forEach(list => {
        if (list._ownerInput) positionAC(list._ownerInput, list);
    });
}, true);
...
async function saveRow(btn) {
    ...
    try {
        const res  = await fetch('pub_express_disbursement.php', { method: 'POST', body: fd });
        const data = await res.json();
        if (data.ok) {
...
async function deleteRow(btn) {
    ...
    const row = btn.closest('tr');
    const id  = row.dataset.id;
    // Any ac-list for this row may have been portalled out to <body>
    // (see openAC/positionAC), so it won't be removed by row.remove() —
    // find it via its stored owner-input reference and clean it up.
    document.querySelectorAll('body > .ac-list').forEach(l => {
        if (l._ownerInput && row.contains(l._ownerInput)) l.remove();
    });
