<?php
// ============================================================
//  commissary_reconcile.php — Commissary Branch Bank Reconciliation
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `commissary_reconcile` (
...
foreach (['ending_balance_books', 'adjusted_bank_balance', 'adjusted_book_balance'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `commissary_reconcile` ADD COLUMN `$newCol` decimal(12,2) DEFAULT NULL");
...
function pullBooksEndingBalance(PDO $pdo, string $store, int $year, int $month): array {
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM commissary_cashflow_balance
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        [$endBooks, ] = pullBooksEndingBalance($pdo, 'Commissary', $fYear, $fMonth);
        $adjBank   = $endBank + $depTrans - $outChecks;
        $adjBook   = $endBooks + $bankCred - $bankChg;
        $variance  = $adjBook - $adjBank;

        $pdo->prepare("
            INSERT INTO commissary_reconcile (store_name, rec_year, rec_month, ending_balance_bank, deposits_in_transit, outstanding_checks, ending_balance_books, bank_credits, bank_charges, adjusted_bank_balance, adjusted_book_balance, saved_by)
            VALUES ('Commissary', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
if (isset($_GET['export_csv'])) {
    $stmt = $pdo->prepare("SELECT * FROM commissary_reconcile WHERE store_name='Commissary' AND rec_year=? AND rec_month=?");
    ...
    [$pulledBooks, ] = pullBooksEndingBalance($pdo, 'Commissary', $fYear, $fMonth);
    ...
    header('Content-Disposition: attachment; filename="Commissary_Reconcile_'.$months[$fMonth-1].'_'.$fYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['BANK RECONCILIATION']);
    fputcsv($out, ['Commissary Branch', $months[$fMonth-1].' '.$fYear]);
    fputcsv($out, []);
...
[$pulledBooks, $booksDebug] = pullBooksEndingBalance($pdo, 'Commissary', $fYear, $fMonth);
...
$pageTitle  = 'Commissary Reconcile';
$activePage = 'commissary_reconcile';
include 'layout.php';
?>
...
<div class="rc-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="commissary_reconcile.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveReconcile() {
  ...
  try {
    const res  = await fetch('commissary_reconcile.php?year='+R_YEAR+'&month='+R_MONTH, {method:'POST', body:fd});
