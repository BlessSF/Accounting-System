<?php
// ============================================================
//  recovery_pdc.php — Stella Branch PDC (Post-Dated Checks)
// ============================================================
...
// ── Auto-create table ─────────────────────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_pdc` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        $stmt = $pdo->prepare("INSERT INTO recovery_pdc (date_issued, amount, saved_by) VALUES (?,?,?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $date)) $date = date('Y-m-d');
    try {
        $pdo->prepare("UPDATE recovery_pdc SET date_issued=?, amount=?, saved_by=? WHERE id=?")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    try {
        $pdo->prepare("DELETE FROM recovery_pdc WHERE id=?")->execute([$id]);
        echo json_encode(['ok' => true]);
    } catch (Throwable $e) {
        echo json_encode(['ok' => false, 'msg' => $e->getMessage()]);
    }
...
if (isset($_GET['export_csv'])) {
    $rows = $pdo->query("SELECT * FROM recovery_pdc ORDER BY date_issued ASC, id ASC")->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="recovery_pdc_' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Recovery Branch — PDC (Post-Dated Checks)']);
...
<div class="section-header">
  <div>
    ...
    <div class="section-subtitle" style="font-family:var(--font-m);font-size:.72rem;color:var(--subtext)">
      Recovery Branch · Post-Dated Checks
    </div>
  </div>
  <a href="recovery_pdc.php?export_csv=1" class="btn btn-ghost btn-sm"
...
async function saveRow(btn) {
  ...
  try {
    const res  = await fetch('recovery_pdc.php', { method: 'POST', body: fd });
