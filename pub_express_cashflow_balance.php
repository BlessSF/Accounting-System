<?php
// pub_express_cashflow_balance.php — Pub Express Branch Cashflow Balance (Cash In / Cash Out / Running Balance)
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `pub_express_cashflow_balance` (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        ...
        $pdo->prepare("INSERT INTO pub_express_cashflow_balance (store_name,txn_date,description,cash_in,cash_out,entry_year,entry_month,created_by) VALUES ('Pub Express',?,?,?,?,?,?,?)")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    try {
        ...
        $in   = (float)($_POST['cash_in']  ?? 0);
        $out  = (float)($_POST['cash_out'] ?? 0);
        $pdo->prepare("UPDATE pub_express_cashflow_balance SET txn_date=?,description=?,cash_in=?,cash_out=? WHERE id=? AND store_name='Pub Express'")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        $id = (int)$_POST['id'];
        $pdo->prepare("DELETE FROM pub_express_cashflow_balance WHERE id=? AND store_name='Pub Express'")
            ->execute([$id]);
        echo json_encode(['ok'=>true]);
    } catch (Throwable $e) {
        echo json_encode(['ok'=>false,'msg'=>$e->getMessage()]);
...
if (isset($_GET['export_csv'])) {
    $rows = $pdo->prepare("SELECT * FROM pub_express_cashflow_balance WHERE store_name='Pub Express' AND entry_year=? AND entry_month=? ORDER BY txn_date ASC, id ASC");
    $rows->execute([$fYear,$fMonth]);
    $rows = $rows->fetchAll(PDO::FETCH_ASSOC);
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="Pub Express_CashflowBalance_'.$monthNames[$fMonth].'_'.$fYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['Pub Express Branch — Cashflow Balance']);
...
$stmt = $pdo->prepare("SELECT * FROM pub_express_cashflow_balance WHERE store_name='Pub Express' AND entry_year=? AND entry_month=? ORDER BY txn_date ASC, id ASC");
...
$activePage = 'pub_express_cashflow_balance';
include 'layout.php';
?>
...
<div class="cb-wrap">
  ...
  <div class="cb-controls">
    ...
    <a href="pub_express_cashflow_balance.php?export_csv=1&month=<?=$fMonth?>&year=<?=$fYear?>" class="btn btn-ghost btn-sm" style="color:var(--accent3);border-color:rgba(251,191,36,.25);background:rgba(251,191,36,.06)">⬇ Download CSV</a>
  </div>
...
async function saveRow(btn) {
  ...
  try {
    const res  = await fetch('pub_express_cashflow_balance.php', {method:'POST', body:fd});
