<?php
// ============================================================
//  h_expenses.php — H Branch Expenses Ledger
...
// Only H branch and management can access
...
// ── Auto-create table ─────────────────────────────────────
$pdo->exec("CREATE TABLE IF NOT EXISTS `h_expenses` (
...
foreach (['pdc','ca','withdrawal','depreciation_expense'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `h_expenses` ADD COLUMN `$newCol` decimal(12,2) DEFAULT 0.00 AFTER `sales_discounts`");
...
foreach (['pest_control_bio_aug','commission_fees','exhaust_cleaning','bank_fees','admin_salary_shares'] as $newCol) {
    try {
        $pdo->exec("ALTER TABLE `h_expenses` ADD COLUMN `$newCol` decimal(12,2) DEFAULT 0.00 AFTER `staff_meal`");
    } catch (Throwable $ignored) {} // column already exists — ignore
}
...
try {
    $pdo->exec("ALTER TABLE `h_expenses` ADD COLUMN `selected_for_cf` tinyint(1) NOT NULL DEFAULT 0 AFTER `row_total`");
} catch (Throwable $ignored) {} // column already exists — ignore
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add'])) {
    ...
    try {
        $sql = "INSERT INTO h_expenses (" . implode(',', array_map(fn($f)=>"`$f`",$fields)) . ") VALUES (" . implode(',', array_fill(0,count($fields),'?')) . ")";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $vals[] = $id;
    try {
        $pdo->prepare("UPDATE h_expenses SET ".implode(',',$sets)." WHERE id=?")->execute($vals);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    $id = (int)($_POST['id'] ?? 0);
    try {
        $pdo->prepare("DELETE FROM h_expenses WHERE id=?")->execute([$id]);
        echo json_encode(['ok'=>true]);
    } catch (Throwable $e) {
        echo json_encode(['ok'=>false,'msg'=>$e->getMessage()]);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save_selected_vat'])) {
    ...
    try {
        $pdo->prepare("
            INSERT INTO h_cf_vat_selection (store_name, sel_year, sel_month, vat_total, row_count, saved_by)
            VALUES ('H', ?, ?, ?, ?, ?)
...
if (isset($_GET['export_csv'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM h_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=? ORDER BY expense_date ASC, id ASC");
    $stmt->execute([$csvYear, $csvMonth]);
    $rows = $stmt->fetchAll();
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="H_Expenses_'.$monthNames[$csvMonth].'_'.$csvYear.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out,['H Branch — Expenses Ledger']);
...
try {
    ...
    $totStmt = $pdo->prepare("SELECT $selectParts FROM h_expenses WHERE YEAR(expense_date)=? AND MONTH(expense_date)=?");
...
try {
    $svStmt = $pdo->prepare("SELECT vat_total, row_count, saved_by FROM h_cf_vat_selection WHERE store_name='H' AND sel_year=? AND sel_month=?");
...
$pageTitle  = 'H Expenses';
$activePage = 'h_expenses';
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title" style="color:#1a1d23">H Branch <span style="color:var(--accent)">Expenses Ledger</span></div>
...
<!-- Row search (Company Name / Particulars) -->
<div class="se-controls" style="margin-top:-6px">
  <input type="text" id="rowSearch" class="form-control" style="max-width:260px"
         placeholder="Search company name or particulars…"
         oninput="filterRows()" onkeydown="if(event.key==='Enter'){event.preventDefault();filterRows();}">
  <button type="button" class="btn btn-ghost btn-sm" onclick="filterRows()">🔍 Search</button>
  <button type="button" class="btn btn-ghost btn-sm" onclick="clearRowSearch()">✕ Clear</button>
  <span id="searchCount" style="font-family:var(--font-m);font-size:.72rem;color:var(--subtext2)"></span>
</div>
...
async function saveSelectedVat(silent) {
  ...
  try {
    const res  = await fetch('h_expenses.php', {method:'POST', body:fd});
...
function doExport(e) {
  ...
  window.location.href = 'h_expenses.php?' + params.toString();
}

// ── Row search (Company Name / Particulars) ─────────────────
function filterRows() {
  const q = document.getElementById('rowSearch').value.trim().toLowerCase();
  const rows = document.querySelectorAll('#se-tbody tr');
  let shown = 0;
  rows.forEach(row => {
    const nameEl = row.querySelector('[data-col="company_name"]');
    const partEl = row.querySelector('[data-col="particulars"]');
    const name = (nameEl?.value || '').toLowerCase();
    const part = (partEl?.value || '').toLowerCase();
    const match = !q || name.includes(q) || part.includes(q);
    row.style.display = match ? '' : 'none';
    if (match) shown++;
  });
  const countEl = document.getElementById('searchCount');
  countEl.textContent = q ? `Showing ${shown} of ${rows.length} row${rows.length===1?'':'s'}` : '';
}
...
async function doAC(inp) {
  ...
  try {
    const res  = await fetch('h_expenses.php?vendor_lookup=1&q=' + encodeURIComponent(q));
    const data = await res.json();
    if (!data.length) {
