<?php
// ============================================================
//  recovery_month_end_inv.php — Recovery Branch Month End Inventory
// ============================================================
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_month_end_inv` (
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_categories` (
    `id`          int(11) NOT NULL AUTO_INCREMENT,
    `store_name`  varchar(50) NOT NULL DEFAULT 'Recovery',
...
$pdo->exec("CREATE TABLE IF NOT EXISTS `recovery_categories_meta` (
...
$alreadySeeded = (bool)$pdo->query("SELECT seeded FROM recovery_categories_meta WHERE store_name='Recovery'")->fetchColumn();
if (!$alreadySeeded) {
    $catCount = (int)$pdo->query("SELECT COUNT(*) FROM recovery_categories WHERE store_name='Recovery'")->fetchColumn();
    if ($catCount === 0) {
        $defaults = ['LIQUORS/BEVERAGES','DRY GOODS','BAR STOCKS','MEAT/WET PRODUCTS',
                     'VEGETABLES','PASTA','CONDIMENTS','BREAD'];
        $insDef = $pdo->prepare("INSERT IGNORE INTO recovery_categories (store_name,name,sort_order) VALUES ('Recovery',?,?)");
        foreach ($defaults as $i => $name) $insDef->execute([$name, $i]);
    }
    $pdo->prepare("INSERT INTO recovery_categories_meta (store_name,seeded) VALUES ('Recovery',1) ON DUPLICATE KEY UPDATE seeded=1")->execute();
}

// One-time self-heal: any category name already used inside
// recovery_month_end_inv (e.g. from a renamed/typo'd section) that isn't
// registered yet gets auto-registered, so its items don't disappear.
$knownNames = $pdo->query("SELECT name FROM recovery_categories WHERE store_name='Recovery'")->fetchAll(PDO::FETCH_COLUMN);
$usedNames  = $pdo->query("SELECT DISTINCT category FROM recovery_month_end_inv WHERE store_name='Recovery' AND category<>''")->fetchAll(PDO::FETCH_COLUMN);
$missing    = array_diff($usedNames, $knownNames);
if (!empty($missing)) {
    $maxOrd = (int)$pdo->query("SELECT COALESCE(MAX(sort_order),-1) FROM recovery_categories WHERE store_name='Recovery'")->fetchColumn();
    $insMiss = $pdo->prepare("INSERT IGNORE INTO recovery_categories (store_name,name,sort_order) VALUES ('Recovery',?,?)");
...
$catStmt = $pdo->query("SELECT name FROM recovery_categories WHERE store_name='Recovery' ORDER BY sort_order,id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        ...
        if ($rowId > 0) {
            $pdo->prepare("UPDATE recovery_month_end_inv SET inv_date=?, inv_year=?, inv_month=?, category=?, sort_order=?, item_desc=?, unit=?, supplier_cost=?, end_inv_num=?, total_amount=?, saved_by=? WHERE id=? AND store_name='Recovery'")
            ...
            $pdo->prepare("INSERT INTO recovery_month_end_inv (inv_date,inv_year,inv_month,store_name,category,sort_order,item_desc,unit,supplier_cost,end_inv_num,total_amount,saved_by) VALUES (?,?,?,'Recovery',?,?,?,?,?,?,?,?)")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM recovery_month_end_inv WHERE id=? AND store_name='Recovery'")->execute([$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_add_category'])) {
    ...
    try {
        ...
        $exists = $pdo->prepare("SELECT id FROM recovery_categories WHERE store_name='Recovery' AND name=?");
        $exists->execute([$name]);
        if ($exists->fetch()) { echo json_encode(['ok'=>false,'msg'=>'A section named "'.$name.'" already exists.']); exit; }
        $maxOrd = (int)$pdo->query("SELECT COALESCE(MAX(sort_order),-1) FROM recovery_categories WHERE store_name='Recovery'")->fetchColumn();
        $pdo->prepare("INSERT INTO recovery_categories (store_name,name,sort_order) VALUES ('Recovery',?,?)")
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_delete_category'])) {
    ...
    try {
        ...
        $chk   = $pdo->prepare("SELECT COUNT(*) FROM recovery_month_end_inv WHERE store_name='Recovery' AND category=?");
        $chk->execute([$name]);
        ...
        if ($itemCount > 0 && $force) {
            $pdo->prepare("DELETE FROM recovery_month_end_inv WHERE store_name='Recovery' AND category=?")->execute([$name]);
        }
        $pdo->prepare("DELETE FROM recovery_categories WHERE store_name='Recovery' AND name=?")->execute([$name]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_duplicate_day'])) {
    ...
    try {
        ...
        $srcStmt = $pdo->prepare("SELECT * FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_date=? ORDER BY sort_order,id");
        ...
        $chkStmt = $pdo->prepare("SELECT COUNT(*) FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_date=?");
        $delStmt = $pdo->prepare("DELETE FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_date=?");
        $insStmt = $pdo->prepare("INSERT INTO recovery_month_end_inv (inv_date,inv_year,inv_month,store_name,category,sort_order,item_desc,unit,supplier_cost,end_inv_num,total_amount,saved_by) VALUES (?,?,?,'Recovery',?,?,?,?,?,?,?,?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_month_counts'])) {
    ...
    try {
        ...
        $cStmt = $pdo->prepare("SELECT DAY(inv_date) as d, COUNT(*) as c FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_year=? AND inv_month=? GROUP BY DAY(inv_date)");
        $cStmt->execute([$yr, $mo]);
...
if (isset($_GET['export_csv'])) {
    $stmt = $pdo->prepare("SELECT * FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_date=? ORDER BY sort_order, id");
    $stmt->execute([$selectedDate]);
    $rows = $stmt->fetchAll();

    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="recovery_month_end_inv_'.date('Y_m',mktime(0,0,0,$fMonth,1,$fYear)).'.csv"');
...
$dayCountStmt = $pdo->prepare("SELECT DAY(inv_date) as d, COUNT(*) as c FROM recovery_month_end_inv WHERE store_name='Recovery' AND inv_year=? AND inv_month=? GROUP BY DAY(inv_date)");
$dayCountStmt->execute([$fYear, $fMonth]);
$dayCounts = array_column($dayCountStmt->fetchAll(PDO::FETCH_ASSOC), 'c', 'd');

$pageTitle  = 'Month End Inv.';
$activePage = 'recovery_month_end_inv';
include 'layout.php';
?>
...
<div class="inv-header">
  <div>
    <div class="eyebrow">Recovery Branch · Inventory</div>
...
<div class="inv-controls">
  <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    ...
    <a href="recovery_month_end_inv.php?export_csv=1&month=<?=$fMonth?>&day=<?=$fDay?>&year=<?=$fYear?>" class="btn btn-ghost">⬇ Download CSV</a>
...
async function saveRow(btn, catName, catIdx) {
  ...
  try {
    const res  = await fetch('recovery_month_end_inv.php', {method:'POST', body:fd});
...
async function deleteRow(btn, catIdx) {
  ...
  if (id > 0) {
    ...
    fd.append('row_id', id);
    try { await fetch('recovery_month_end_inv.php', {method:'POST', body:fd}); } catch(e) {}
  }
