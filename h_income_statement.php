<?php
// ============================================================
//  h_income_statement.php — H Hero Income Statement
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_save'])) {
    ...
    try {
        $sql = "INSERT INTO h_income_statement ("
...
$row = null;
try {
    $stmt = $pdo->prepare("SELECT * FROM h_income_statement WHERE store_name='H' AND stmt_date=? LIMIT 1");
...
// Net Sales = SUM(store_gross) for the selected month
$expYear  = (int)date('Y', strtotime($stmtDate));
$expMonth = (int)date('n', strtotime($stmtDate));
$storeGrossTotal = 0;
try {
    $sgStmt = $pdo->prepare("SELECT COALESCE(SUM(`store_gross`),0) FROM `h_report_entries` WHERE store_name='H' AND YEAR(report_date)=? AND MONTH(report_date)=?");
...
// ── Auto Sales Discount = total Sales Discounts from expenses ledger ─
$salesDiscountAuto = (float)($expTotals['sales_discounts'] ?? 0);
$salesDiscountAutoFmt = number_format($salesDiscountAuto, 2, '.', '');
...
<div class="is-card">
  ...
  <div class="is-body">
    ...
    <!-- Sales Discount — auto-pulled from Expenses Ledger Sales Discounts total -->
    <div class="is-row">
      <div class="is-label">
        Sales Discount
        <span style="font-family:var(--font-m);font-size:.6rem;color:var(--accent);margin-left:8px;
      ...
      <div class="is-input-wrap">
        <input type="number" step="0.01" class="is-input" id="sales_discount" data-field="sales_discount"
               value="<?= $salesDiscountAutoFmt ?>"
               readonly tabindex="-1"
               style="background:#f0fdf4;color:var(--accent);font-weight:700;cursor:default;border-color:rgba(15,123,92,.2)">
        <div class="is-pct red" id="pct_sales_discount">—</div>
      </div>
    </div>
