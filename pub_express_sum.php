<?php
// ============================================================
//  pub_express_sum.php — Pub Express Sum / Checking View
//  Mirrors the Excel "Checking" tab format:
//  ASSETS + EXPENSES rows × Jan–Dec months with TOTAL column
//  Data pulled live from pub_express_disbursement
// ============================================================
...
if (isset($_GET['export_csv'])) {
    ...
    // Account title rows now come from pub_express_acc_titles (managed via
    // pub_express_acc_title.php) instead of a hardcoded list, grouped by
    // section. Any account_title found in disbursement data that's no longer
    // in the masterlist (renamed/deleted) is still included under Expenses
    // so historical totals are never silently dropped.
    $acctRows = $pdo->query("SELECT title, section FROM `pub_express_acc_titles` ORDER BY sort_order ASC, title ASC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    header('Content-Disposition: attachment; filename="pub_express_sum_'.$YEAR.'.csv"');
    $out = fopen('php://output','w');
    fputcsv($out, ['Pub Express — Sum / Checking', $YEAR]);
    fputcsv($out, []);
...
// Account title rows now come from pub_express_acc_titles (managed via
// pub_express_acc_title.php) instead of a hardcoded list, grouped by
// section. Any account_title found in disbursement data that's no longer in
// the masterlist (renamed/deleted) is still included under Expenses so
...
$pageTitle  = 'Pub Express Sum';
$activePage = 'pub_express_disbursement'; // keep Disbursement active in sidebar
include 'layout.php';
?>
...
<div class="section-header">
  <div>
    <div class="section-title">Pub Express <span>Sum</span></div>
...
<div class="sum-controls">
  ...
  <a href="?year=<?= $YEAR+1 ?>" class="yr-btn"><?= $YEAR+1 ?> →</a>
  <a href="pub_express_disbursement.php" class="btn-action">📒 Go to Disbursement</a>
  <a href="pub_express_acc_title.php" class="btn-action" style="border-color:#6D4C41;background:#EFEBE9;color:#4E342E">🏷 Account Titles</a>
